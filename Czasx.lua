...
================================================================================
                              C Z A S X
                   Admin Command Suite  |  Build 2.4.1-cz
--------------------------------------------------------------------------------
  Author     : Czasx Engine
  Type       : Local FE command bar
  Prefix     : ;   (change in settings)
  Config     : czasx_cfg.dat
--------------------------------------------------------------------------------
  USAGE
    1. Execute this script in your executor
    2. Panel opens bottom-left
    3. Type command + Enter  (mode EXEC)
    4. Click EXEC/FIND to toggle search-only mode
    5. FIND mode = filter list only, Enter does not run cmds
--------------------------------------------------------------------------------
  MODE BUTTON
    EXEC  -> run commands on Enter
    FIND  -> search/filter only
--------------------------------------------------------------------------------
  COMMON COMMANDS (default czPrefix ;)
    ;fly [speed]           fly mode
    ;unfly                 stop fly
    ;noclip                walk through walls
    ;clip                  stop noclip
    ;esp / ;noesp          player ESP on/off
    ;goto [player]         teleport to player
    ;speed [num]           walkspeed
    ;jump [num]            jumppower
    ;rejoin                rejoin same server
    ;serverhop             hop to another server
    ;freecam / ;unfreecam  free camera
    ;view [player]         spectate
    ;unview                stop spectate
    ;invisible             client invis
    ;btools                building tools (client)
    ;console               roblox developer console
    ;dex                   explorer
    Full list: type partial name in FIND mode to browse (~400 cmds)
--------------------------------------------------------------------------------
  WATERMARK  |  Czasx  |  Not affiliated with any other admin tool
================================================================================
]]

-- BOOTGUARD: block double injection
if _G.CZASX_BOOT and not _G.CZASX_VERBOSE then
	-- error("Czasx already active", 0)
	return
end

pcall(function() getgenv().CZASX_BOOT = true; _G.CZASX_BOOT = true end)
if not game:IsLoaded() then game.Loaded:Wait() end

-- UTIL: resolve exploit API or fallback
function czSafeFn(t, f, fallback)
	if type(f) == t then return f end
	return fallback
end

cloneref = czSafeFn("function", cloneref, function(...) return ... end)
sethidden =  czSafeFn("function", sethiddenproperty or set_hidden_property or set_hidden_prop)
gethidden =  czSafeFn("function", gethiddenproperty or get_hidden_property or get_hidden_prop)
queueteleport =  czSafeFn("function", queue_on_teleport or (syn and syn.queue_on_teleport) or (fluxus and fluxus.queue_on_teleport))
httprequest =  czSafeFn("function", request or http_request or (syn and syn.request) or (http and http.request) or (fluxus and fluxus.request))
everyClipboard = czSafeFn("function", setclipboard or toclipboard or set_clipboard or (Clipboard and Clipboard.set))
firetouchinterest = czSafeFn("function", firetouchinterest)
waxwritefile, waxreadfile = writefile, readfile
writefile = czSafeFn("function", waxwritefile) and function(file, data, safe)
	if safe == true then return pcall(waxwritefile, file, data) end
	waxwritefile(file, data)
end
readfile = czSafeFn("function", waxreadfile) and function(file, safe)
	if safe == true then return pcall(waxreadfile, file) end
	return waxreadfile(file)
end
isfile = czSafeFn("function", isfile, readfile and function(file)
	local success, result = pcall(function()
		return readfile(file)
	end)
	return success and result ~= nil and result ~= ""
end)
makefolder = czSafeFn("function", makefolder)
isfolder = czSafeFn("function", isfolder)
waxgetcustomasset = czSafeFn("function", czAsset or getsynasset)
hookfunction = czSafeFn("function", hookfunction)
hookmetamethod = czSafeFn("function", hookmetamethod)
getnamecallmethod = czSafeFn("function", getnamecallmethod or get_namecall_method)
checkcaller = czSafeFn("function", checkcaller, function() return false end)
newcclosure = czSafeFn("function", newcclosure, function(f, ...) return f(...) end)
getgc = czSafeFn("function", getgc or get_gc_objects)
setthreadidentity = czSafeFn("function", setthreadidentity or (syn and syn.set_thread_identity) or syn_context_set or setthreadcontext)
replicatesignal = czSafeFn("function", replicatesignal)
getconnections = czSafeFn("function", getconnections or get_signal_cons)

-- SERVICES: lazy-cached GetService wrapper
CzServices = setmetatable({}, {
...
Players = CzServices.Players
UserInputService = CzServices.UserInputService
TweenService = CzServices.TweenService
HttpService = CzServices.HttpService
ExperienceService = CzServices.ExperienceService
MarketplaceService = CzServices.MarketplaceService
RunService = CzServices.RunService
TeleportService = CzServices.TeleportService
StarterGui = CzServices.StarterGui
GuiService = CzServices.GuiService
Lighting = CzServices.Lighting
ContextActionService = CzServices.ContextActionService
ReplicatedStorage = CzServices.ReplicatedStorage
GroupService = CzServices.GroupService
PathService = CzServices.PathfindingService
SoundService = CzServices.SoundService
Teams = CzServices.Teams
StarterPlayer = CzServices.StarterPlayer
InsertService = CzServices.InsertService
ChatService = CzServices.Chat
ProximityPromptService = CzServices.ProximityPromptService
ContentProvider = CzServices.ContentProvider
StatsService = CzServices.Stats
MaterialService = CzServices.MaterialService
AvatarEditorService = CzServices.AvatarEditorService
TextService = CzServices.TextService
TextChatService = CzServices.TextChatService
CaptureService = CzServices.CaptureService
VoiceChatService = CzServices.VoiceChatService
SocialService = CzServices.SocialService

CzPlayerGui = cloneref(Players.LocalPlayer:FindFirstChildWhichIsA("CzPlayerGui"))
CzCore = CzServices.CoreGui or CzPlayerGui
CzMouse = cloneref(Players.LocalPlayer:GetMouse())
PlaceId, JobId = game.PlaceId, game.JobId
xpcall(function()
	czMobile = table.find({Enum.Platform.Android, Enum.Platform.IOS}, UserInputService:GetPlatform())
end, function()
	czMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
end)
czLegacyChat = TextChatService.ChatVersion == Enum.ChatVersion.LegacyChatService
...
-- Czasx core
local czAssetMap = {
	["czasx/res/bindsandplugins.png"] = "rbxassetid://5147695474",
	["czasx/res/close.png"] = "rbxassetid://5054663650",
	["czasx/res/editaliases.png"] = "rbxassetid://5147488658",
	["czasx/res/editkeybinds.png"] = "rbxassetid://129697930",
	["czasx/res/edittheme.png"] = "rbxassetid://4911962991",
	["czasx/res/editwaypoints.png"] = "rbxassetid://5147488592",
	["czasx/res/imgstudiopluginlogo.png"] = "rbxassetid://4113050383",
	["czasx/res/logo.png"] = "rbxassetid://1352543873",
	["czasx/res/minimize.png"] = "rbxassetid://2406617031",
	["czasx/res/pin.png"] = "rbxassetid://6234691350",
	["czasx/res/reference.png"] = "rbxassetid://3523243755",
	["czasx/res/settings.png"] = "rbxassetid://1204397029"
}

local function czAsset(asset)
...
if makefolder and isfolder and writefile and isfile then
	pcall(function() -- good executor trust
		...
		for _, folder in {"czasx", "czasx/res"} do
			if not isfolder(folder) then
				makefolder(folder)
			end
		end
		for path in czAssetMap do
			if not isfile(path) then
				writefile(path, game:HttpGet((path:gsub("czasx/res/", assets))))
			end
		end
		-- honestly just blame your phone if the assets appear in your gallery
		if czMobile then writefile("czasx/res/.nomedia", "") end
	end)
end

CZ_VER = "2.4.1-cz"

CzRootScale = Instance.new("Frame")
Scale = Instance.new("UIScale")
CzPanel = Instance.new("Frame")
Title = Instance.new("TextLabel")
Dark = Instance.new("Frame")
CzInput = Instance.new("TextBox")
CzList = Instance.new("ScrollingFrame")
CzListLayout = Instance.new("UIListLayout")
CzGearBtn = Instance.new("ImageButton")
CzThemeBtn = Instance.new("ImageButton")
CzSettings = Instance.new("Frame")
...
elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
    ...
    Main.Parent = CzCore
    CzGuiParent = Main
elseif CzCore:FindFirstChild("RobloxGui") then
    CzGuiParent = CzCore.RobloxGui
else
    local Main = Instance.new("ScreenGui")
    Main.Name = czRandName()
    Main.ResetOnSpawn = false
...
CzRootScale.Name = czRandName()
CzRootScale.Size = UDim2.fromScale(1, 1)
CzRootScale.BackgroundTransparency = 1
CzRootScale.Parent = CzGuiParent
Scale.Name = czRandName()

CzPanel.Name = czRandName()
CzPanel.Parent = CzRootScale
CzPanel.Active = true
CzPanel.BackgroundColor3 = Color3.fromRGB(12, 14, 20)
CzPanel.BorderSizePixel = 0
CzPanel.Position = UDim2.new(0, 18, 1, -275)
CzPanel.Size = UDim2.new(0, 280, 0, 260)
CzPanel.ClipsDescendants = true
CzPanel.ZIndex = 10
table.insert(czShadeB,CzPanel)

Title.Name = "Title"
Title.Parent = CzPanel
Title.Active = true
Title.BackgroundColor3 = Color3.fromRGB(8, 10, 16)
Title.BorderSizePixel = 0
Title.Size = UDim2.new(0, 280, 0, 28)
Title.Font = Enum.Font.GothamBold
Title.TextSize = 13
Title.TextXAlignment = Enum.TextXAlignment.Left
Title.Text = "  Czasx  ·  " .. CZ_VER
...
Title.ZIndex = 10
table.insert(czShadeA,Title)
table.insert(czTextA,Title)


-- UI LAYER: FIND/EXEC toggle + panel chrome
-- Czasx interaction layer
_G.CZ_SEARCH = false
czFindMode = false

CzModeBtn = Instance.new("TextButton")
CzModeBtn.Name = czRandName()
CzModeBtn.Parent = CzPanel
CzModeBtn.BackgroundColor3 = Color3.fromRGB(20, 28, 38)
CzModeBtn.BorderSizePixel = 0
CzModeBtn.Position = UDim2.new(0, 222, 0, 30)
CzModeBtn.Size = UDim2.new(0, 50, 0, 22)
CzModeBtn.Font = Enum.Font.GothamBold
CzModeBtn.TextSize = 10
CzModeBtn.Text = "EXEC"
CzModeBtn.TextColor3 = Color3.fromRGB(80, 220, 200)
CzModeBtn.ZIndex = 12
CzModeBtn.AutoButtonColor = false
do
	local mbC = Instance.new("UICorner")
	mbC.CornerRadius = UDim.new(0, 5)
	mbC.Parent = CzModeBtn
	local mbS = Instance.new("UIStroke")
	mbS.Name = "Stroke"
	mbS.Color = Color3.fromRGB(40, 160, 150)
	mbS.Thickness = 1
	mbS.Transparency = 0.4
	mbS.Parent = CzModeBtn
end

function czSyncMode()
	local stroke = CzModeBtn:FindFirstChild("Stroke")
	if czFindMode then
		CzModeBtn.Text = "FIND"
		CzModeBtn.BackgroundColor3 = Color3.fromRGB(28, 18, 48)
		CzModeBtn.TextColor3 = Color3.fromRGB(200, 160, 255)
		if stroke then stroke.Color = Color3.fromRGB(140, 90, 220) end
		CzInput.PlaceholderText = "search..."
		CzInput.PlaceholderColor3 = Color3.fromRGB(130, 110, 180)
	else
		CzModeBtn.Text = "EXEC"
		CzModeBtn.BackgroundColor3 = Color3.fromRGB(20, 28, 38)
		CzModeBtn.TextColor3 = Color3.fromRGB(80, 220, 200)
		if stroke then stroke.Color = Color3.fromRGB(40, 160, 150) end
		CzInput.PlaceholderText = "type command..."
		CzInput.PlaceholderColor3 = Color3.fromRGB(90, 100, 130)
	end
	_G.CZ_SEARCH = czFindMode
end

CzModeBtn.MouseButton1Click:Connect(function()
	czFindMode = not czFindMode
	czSyncMode()
	CzInput.Text = ""
	pcall(function() czFilterList('', true, false, true) end)
end)
CzModeBtn.MouseEnter:Connect(function()
	TweenService:Create(CzModeBtn, TweenInfo.new(0.12), {
		BackgroundColor3 = czFindMode and Color3.fromRGB(40, 25, 70) or Color3.fromRGB(28, 40, 52)
	}):Play()
end)
CzModeBtn.MouseLeave:Connect(function()
	TweenService:Create(CzModeBtn, TweenInfo.new(0.12), {
		BackgroundColor3 = czFindMode and Color3.fromRGB(28, 18, 48) or Color3.fromRGB(20, 28, 38)
	}):Play()
end)
...
do
	...
	hs.Color = Color3.fromRGB(30, 140, 130)
	hs.Thickness = 1.2
...
Dark.Parent = CzPanel
Dark.Active = true
Dark.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
Dark.BorderSizePixel = 0
Dark.Position = UDim2.new(0, 0, 0, 56)
Dark.Size = UDim2.new(0, 280, 0, 204)
Dark.ZIndex = 10
table.insert(czShadeA,Dark)

CzInput.Name = czRandName()
CzInput.Parent = CzPanel
CzInput.BackgroundColor3 = Color3.fromRGB(18, 22, 32)
CzInput.BackgroundTransparency = 0.15
CzInput.BorderSizePixel = 0
CzInput.Position = UDim2.new(0, 8, 0, 30)
CzInput.Size = UDim2.new(0, 210, 0, 22)
CzInput.Font = Enum.Font.Gotham
CzInput.TextSize = 13
CzInput.TextXAlignment = Enum.TextXAlignment.Left
CzInput.TextColor3 = Color3.fromRGB(220, 230, 255)
CzInput.Text = ""
CzInput.ZIndex = 10
CzInput.PlaceholderText = "type command..."
CzInput.PlaceholderColor3 = Color3.fromRGB(90, 100, 130)
...
CzList.Name = czRandName()
CzList.Parent = CzPanel
CzList.BackgroundTransparency = 1
CzList.BorderSizePixel = 0
CzList.Position = UDim2.new(0, 6, 0, 56)
CzList.Size = UDim2.new(0, 268, 0, 200)
CzList.ScrollBarImageColor3 = Color3.fromRGB(40, 180, 170)
CzList.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
CzList.CanvasSize = UDim2.new(0, 0, 0, 0)
CzList.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
CzList.ScrollBarThickness = 4
CzList.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
CzList.VerticalScrollBarInset = 'Always'
CzList.ZIndex = 10
CzList.ScrollBarImageTransparency = 0.35
table.insert(czScrollBag,CzList)

CzListLayout.Parent = CzList

CzGearBtn.Name = "CzGearBtn"
CzGearBtn.Parent = CzPanel
CzGearBtn.BackgroundTransparency = 1
CzGearBtn.Position = UDim2.new(0, 254, 0, 4)
CzGearBtn.Size = UDim2.new(0, 22, 0, 22)
CzGearBtn.Image = czAsset("czasx/res/settings.png")
CzGearBtn.ZIndex = 10

CzRefBtn = Instance.new("ImageButton")
CzRefBtn.Name = "CzRefBtn"
CzRefBtn.Parent = CzPanel
CzRefBtn.BackgroundTransparency = 1
CzRefBtn.Position = UDim2.new(0, 230, 0, 5)
CzRefBtn.Size = UDim2.new(0, 16, 0, 16)
CzRefBtn.Image = czAsset("czasx/res/reference.png")
CzRefBtn.ZIndex = 10

CzSettings.Name = "CzSettings"
CzSettings.Parent = CzPanel
CzSettings.Active = true
CzSettings.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
CzSettings.BorderSizePixel = 0
CzSettings.Position = UDim2.new(0, 0, 0, 260)
CzSettings.Size = UDim2.new(0, 280, 0, 204)
CzSettings.ZIndex = 10
table.insert(czShadeA,CzSettings)

SettingsHolder = Instance.new("ScrollingFrame")
SettingsHolder.Name = "CzPanel"
SettingsHolder.Parent = CzSettings
SettingsHolder.BackgroundTransparency = 1
SettingsHolder.BorderSizePixel = 0
SettingsHolder.Size = UDim2.new(1,0,1,0)
SettingsHolder.ScrollBarImageColor3 = Color3.fromRGB(35, 150, 140)
SettingsHolder.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
SettingsHolder.CanvasSize = UDim2.new(0, 0, 0, 235)
SettingsHolder.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
SettingsHolder.ScrollBarThickness = 8
SettingsHolder.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
SettingsHolder.VerticalScrollBarInset = 'Always'
SettingsHolder.ZIndex = 10
table.insert(czScrollBag,SettingsHolder)

Prefix.Name = "Prefix"
Prefix.Parent = SettingsHolder
Prefix.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Prefix.ZIndex = 10
table.insert(czShadeB,Prefix)
table.insert(czTextA,Prefix)

PrefixBox.Name = "PrefixBox"
PrefixBox.Parent = Prefix
PrefixBox.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
PrefixBox.ZIndex = 10
table.insert(czShadeC,PrefixBox)
table.insert(czTextB,PrefixBox)
...
function makeSettingsButton(name,iconID,off)
	...
	table.insert(czShadeB,button)
	table.insert(czTextA,label)
	return button
end

CzThemeBtn = makeSettingsButton("Appearance",czAsset("czasx/res/edittheme.png"))
CzThemeBtn.Position = UDim2.new(0, 5, 0, 55)
CzThemeBtn.Size = UDim2.new(1, -10, 0, 25)
CzThemeBtn.Name = "Colors"
CzThemeBtn.Parent = SettingsHolder

Keybinds = makeSettingsButton("Hotkeys",czAsset("czasx/res/editkeybinds.png"))
Keybinds.Position = UDim2.new(0, 5, 0, 85)
Keybinds.Size = UDim2.new(1, -10, 0, 25)
Keybinds.Name = "Keybinds"
Keybinds.Parent = SettingsHolder

Aliases = makeSettingsButton("Aliases",czAsset("czasx/res/editaliases.png"))
...
StayOpen.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
StayOpen.ZIndex = 10
table.insert(czShadeB,StayOpen)
table.insert(czTextA,StayOpen)

Button.Name = "Button"
Button.Parent = StayOpen
Button.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
Button.BorderSizePixel = 0
Button.Position = UDim2.new(1, -20, 0, 0)
Button.Size = UDim2.new(0, 20, 0, 20)
Button.ZIndex = 10
table.insert(czShadeC,Button)
...
Positions = makeSettingsButton("Waypoints",czAsset("czasx/res/editwaypoints.png"))
Positions.Position = UDim2.new(0, 5, 0, 145)
Positions.Size = UDim2.new(1, -10, 0, 25)
Positions.Name = "Waypoints"
Positions.Parent = SettingsHolder

EventBind = makeSettingsButton("Events",czAsset("czasx/res/bindsandplugins.png"),759)
EventBind.Position = UDim2.new(0, 5, 0, 205)
EventBind.Size = UDim2.new(1, -10, 0, 25)
EventBind.Name = "EventBinds"
EventBind.Parent = SettingsHolder

Plugins = makeSettingsButton("Plugins",czAsset("czasx/res/bindsandplugins.png"),743)
...
Example.ZIndex = 10
table.insert(czTextA,Example)

Notification.Name = czRandName()
Notification.Parent = CzRootScale
Notification.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
Notification.BorderSizePixel = 0
Notification.Position = UDim2.new(1, -500, 1, 20)
Notification.Size = UDim2.new(0, 280, 0, 100)
Notification.ZIndex = 10
table.insert(czShadeA,Notification)

Title_2.Name = "Title"
Title_2.Parent = Notification
Title_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Title_2.BorderSizePixel = 0
Title_2.Size = UDim2.new(0, 280, 0, 22)
Title_2.Font = Enum.Font.SourceSans
Title_2.TextSize = 14
Title_2.Text = "Notification Title"
Title_2.TextColor3 = Color3.new(1, 1, 1)
Title_2.ZIndex = 10
table.insert(czShadeB,Title_2)
table.insert(czTextA,Title_2)
...
CloseImage.Image = czAsset("czasx/res/close.png")
...
PinImage.ZIndex = 10
PinImage.Image = czAsset("czasx/res/pin.png")

Tooltip.Name = czRandName()
Tooltip.Parent = CzRootScale
Tooltip.Active = true
Tooltip.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
Tooltip.BackgroundTransparency = 0.1
Tooltip.BorderSizePixel = 0
Tooltip.Size = UDim2.new(0, 200, 0, 96)
Tooltip.Visible = false
Tooltip.ZIndex = 10
table.insert(czShadeA,Tooltip)

Title_3.Name = "Title"
Title_3.Parent = Tooltip
Title_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Title_3.ZIndex = 10
table.insert(czShadeB,Title_3)
table.insert(czTextA,Title_3)
...
table.insert(czTextA,Description)

IntroBackground.Name = "IntroBackground"
IntroBackground.Parent = CzPanel
IntroBackground.Active = true
IntroBackground.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
IntroBackground.BorderSizePixel = 0
IntroBackground.Position = UDim2.new(0, 0, 0, 45)
IntroBackground.Size = UDim2.new(0, 280, 0, 204)
...
Logo.Image = czAsset("czasx/res/logo.png")
Logo.ImageTransparency = 0
Logo.ZIndex = 10

Credits.Name = "Credits"
Credits.Parent = CzPanel
Credits.BackgroundTransparency = 1
Credits.BorderSizePixel = 0
Credits.Position = UDim2.new(0, 0, 0.9, 30)
Credits.Size = UDim2.new(0, 280, 0, 20)
...
KeybindsFrame.Parent = CzSettings
KeybindsFrame.Active = true
KeybindsFrame.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
KeybindsFrame.BorderSizePixel = 0
KeybindsFrame.Position = UDim2.new(0, 0, 0, 175)
KeybindsFrame.Size = UDim2.new(0, 280, 0, 204)
KeybindsFrame.ZIndex = 10
table.insert(czShadeA,KeybindsFrame)

Close.Name = "Close"
Close.Parent = KeybindsFrame
Close.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Close.ZIndex = 10
table.insert(czShadeB,Close)
table.insert(czTextA,Close)

Add.Name = "Add"
Add.Parent = KeybindsFrame
Add.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Add.ZIndex = 10
table.insert(czShadeB,Add)
table.insert(czTextA,Add)

Delete.Name = "Delete"
Delete.Parent = KeybindsFrame
Delete.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Delete.ZIndex = 10
table.insert(czShadeB,Delete)
table.insert(czTextA,Delete)

Holder_2.Name = "CzPanel"
Holder_2.Parent = KeybindsFrame
Holder_2.BackgroundTransparency = 1
Holder_2.BorderSizePixel = 0
Holder_2.Position = UDim2.new(0, 0, 0, 0)
Holder_2.Size = UDim2.new(0, 280, 0, 170)
Holder_2.ScrollBarImageColor3 = Color3.fromRGB(35, 150, 140)
Holder_2.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_2.CanvasSize = UDim2.new(0, 0, 0, 0)
Holder_2.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_2.ScrollBarThickness = 0
Holder_2.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_2.VerticalScrollBarInset = 'Always'
Holder_2.ZIndex = 10

Example_2.Name = "Example"
Example_2.Parent = KeybindsFrame
Example_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Example_2.BorderSizePixel = 0
Example_2.Size = UDim2.new(0, 10, 0, 20)
Example_2.Visible = false
Example_2.ZIndex = 10
table.insert(czShadeB,Example_2)

Text_3.Name = "Text"
Text_3.Parent = Example_2
Text_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Text_3.ZIndex = 10
table.insert(czShadeB,Text_3)
table.insert(czTextA,Text_3)

Delete_2.Name = "Delete"
Delete_2.Parent = Text_3
Delete_2.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
Delete_2.ZIndex = 10
table.insert(czShadeC,Delete_2)
table.insert(czTextB,Delete_2)

KeybindEditor.Name = czRandName()
KeybindEditor.Parent = CzRootScale
...
background_2.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
background_2.BorderSizePixel = 0
background_2.Position = UDim2.new(0, 0, 0, 20)
background_2.Size = UDim2.new(0, 360, 0, 185)
background_2.ZIndex = 10
table.insert(czShadeA,background_2)

Dark_3.Name = "Dark"
Dark_3.Parent = background_2
Dark_3.Active = true
Dark_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Dark_3.BorderSizePixel = 0
Dark_3.Position = UDim2.new(0, 135, 0, 0)
Dark_3.Size = UDim2.new(0, 2, 0, 185)
Dark_3.ZIndex = 10
table.insert(czShadeB,Dark_3)
...
table.insert(czTextA,Directions)

BindTo.Name = "BindTo"
BindTo.Parent = background_2
BindTo.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,BindTo)
table.insert(czTextA,BindTo)
...
table.insert(czTextA,TriggerLabel)

BindTriggerSelect.Name = "BindTo"
BindTriggerSelect.Parent = background_2
BindTriggerSelect.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,BindTriggerSelect)
table.insert(czTextA,BindTriggerSelect)

Add_2.Name = "Add"
Add_2.Parent = background_2
Add_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,Add_2)
table.insert(czTextA,Add_2)
...
Toggles.ZIndex = 10
Toggles.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Toggles.CanvasSize = UDim2.new(0, 0, 0, 50)
Toggles.ScrollBarThickness = 8
Toggles.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Toggles.VerticalScrollBarInset = Enum.ScrollBarInset.Always
table.insert(czScrollBag,Toggles)

ClickTP.Name = "Click TP (Hold Key & Click)"
ClickTP.Parent = Toggles
ClickTP.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,ClickTP)
table.insert(czTextA,ClickTP)

Select.Name = "Select"
Select.Parent = ClickTP
Select.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
table.insert(czShadeC,Select)
table.insert(czTextB,Select)

ClickDelete.Name = "Click Delete (Hold Key & Click)"
ClickDelete.Parent = Toggles
ClickDelete.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,ClickDelete)
table.insert(czTextA,ClickDelete)

Select_2.Name = "Select"
Select_2.Parent = ClickDelete
Select_2.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
table.insert(czShadeC,Select_2)
table.insert(czTextB,Select_2)

Cmdbar_2.Name = "Cmdbar_2"
Cmdbar_2.Parent = background_2
Cmdbar_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Cmdbar_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
CreateToggle.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czTextA,CreateToggle)

Button_2.Name = "Button"
Button_2.Parent = CreateToggle
Button_2.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
Button_2.BorderSizePixel = 0
Button_2.Position = UDim2.new(1, -20, 0, 0)
Button_2.Size = UDim2.new(0, 20, 0, 20)
Button_2.ZIndex = 10
table.insert(czShadeC,Button_2)
...
shadow_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
shadow_2.BorderSizePixel = 0
shadow_2.Size = UDim2.new(0, 360, 0, 20)
shadow_2.ZIndex = 10
table.insert(czShadeB,shadow_2)
...
table.insert(czTextA,PopupText_2)
...
ExitImage_2.Image = czAsset("czasx/res/close.png")

PositionsFrame.Name = "PositionsFrame"
PositionsFrame.Parent = CzSettings
PositionsFrame.Active = true
PositionsFrame.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
PositionsFrame.BorderSizePixel = 0
PositionsFrame.Size = UDim2.new(0, 280, 0, 204)
PositionsFrame.Position = UDim2.new(0, 0, 0, 175)
PositionsFrame.ZIndex = 10
table.insert(czShadeA,PositionsFrame)

Close_3.Name = "Close"
Close_3.Parent = PositionsFrame
Close_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Close_3.ZIndex = 10
table.insert(czShadeB,Close_3)
table.insert(czTextA,Close_3)

Delete_5.Name = "Delete"
Delete_5.Parent = PositionsFrame
Delete_5.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Delete_5.ZIndex = 10
table.insert(czShadeB,Delete_5)
table.insert(czTextA,Delete_5)

Part.Name = "PartGoto"
Part.Parent = PositionsFrame
Part.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Part.ZIndex = 10
table.insert(czShadeB,Part)
table.insert(czTextA,Part)

Holder_4.Name = "CzPanel"
Holder_4.Parent = PositionsFrame
Holder_4.BackgroundTransparency = 1
Holder_4.BorderSizePixel = 0
Holder_4.Position = UDim2.new(0, 0, 0, 0)
Holder_4.Selectable = false
Holder_4.Size = UDim2.new(0, 280, 0, 170)
Holder_4.ScrollBarImageColor3 = Color3.fromRGB(35, 150, 140)
Holder_4.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_4.CanvasSize = UDim2.new(0, 0, 0, 0)
Holder_4.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_4.ScrollBarThickness = 0
Holder_4.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_4.VerticalScrollBarInset = 'Always'
Holder_4.ZIndex = 10

Example_4.Name = "Example"
Example_4.Parent = PositionsFrame
Example_4.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Example_4.BorderSizePixel = 0
Example_4.Size = UDim2.new(0, 10, 0, 20)
Example_4.Visible = false
Example_4.Position = UDim2.new(0, 0, 0, -5)
Example_4.ZIndex = 10
table.insert(czShadeB,Example_4)

Text_5.Name = "Text"
Text_5.Parent = Example_4
Text_5.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Text_5.ZIndex = 10
table.insert(czShadeB,Text_5)
table.insert(czTextA,Text_5)

Delete_6.Name = "Delete"
Delete_6.Parent = Text_5
Delete_6.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
Delete_6.ZIndex = 10
table.insert(czShadeC,Delete_6)
table.insert(czTextB,Delete_6)

TP.Name = "TP"
TP.Parent = Text_5
TP.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
TP.ZIndex = 10
table.insert(czShadeC,TP)
table.insert(czTextB,TP)

AliasesFrame.Name = "AliasesFrame"
AliasesFrame.Parent = CzSettings
AliasesFrame.Active = true
AliasesFrame.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
AliasesFrame.BorderSizePixel = 0
AliasesFrame.Position = UDim2.new(0, 0, 0, 175)
AliasesFrame.Size = UDim2.new(0, 280, 0, 204)
AliasesFrame.ZIndex = 10
table.insert(czShadeA,AliasesFrame)

Close_2.Name = "Close"
Close_2.Parent = AliasesFrame
Close_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Close_2.ZIndex = 10
table.insert(czShadeB,Close_2)
table.insert(czTextA,Close_2)

Delete_3.Name = "Delete"
Delete_3.Parent = AliasesFrame
Delete_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Delete_3.ZIndex = 10
table.insert(czShadeB,Delete_3)
table.insert(czTextA,Delete_3)

Holder_3.Name = "CzPanel"
Holder_3.Parent = AliasesFrame
Holder_3.BackgroundTransparency = 1
Holder_3.BorderSizePixel = 0
Holder_3.Position = UDim2.new(0, 0, 0, 0)
Holder_3.Size = UDim2.new(0, 280, 0, 170)
Holder_3.ScrollBarImageColor3 = Color3.fromRGB(35, 150, 140)
Holder_3.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_3.CanvasSize = UDim2.new(0, 0, 0, 0)
Holder_3.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_3.ScrollBarThickness = 0
Holder_3.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_3.VerticalScrollBarInset = 'Always'
Holder_3.ZIndex = 10

Example_3.Name = "Example"
Example_3.Parent = AliasesFrame
Example_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Example_3.BorderSizePixel = 0
Example_3.Size = UDim2.new(0, 10, 0, 20)
Example_3.Visible = false
Example_3.ZIndex = 10
table.insert(czShadeB,Example_3)

Text_4.Name = "Text"
Text_4.Parent = Example_3
Text_4.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Text_4.ZIndex = 10
table.insert(czShadeB,Text_4)
table.insert(czTextA,Text_4)

Delete_4.Name = "Delete"
Delete_4.Parent = Text_4
Delete_4.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
Delete_4.ZIndex = 10
table.insert(czShadeC,Delete_4)
table.insert(czTextB,Delete_4)

PluginsFrame.Name = "PluginsFrame"
PluginsFrame.Parent = CzSettings
PluginsFrame.Active = true
PluginsFrame.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
PluginsFrame.BorderSizePixel = 0
PluginsFrame.Position = UDim2.new(0, 0, 0, 175)
PluginsFrame.Size = UDim2.new(0, 280, 0, 204)
PluginsFrame.ZIndex = 10
table.insert(czShadeA,PluginsFrame)

Close_4.Name = "Close"
Close_4.Parent = PluginsFrame
Close_4.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Close_4.ZIndex = 10
table.insert(czShadeB,Close_4)
table.insert(czTextA,Close_4)

Add_3.Name = "Add"
Add_3.Parent = PluginsFrame
Add_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Add_3.ZIndex = 10
table.insert(czShadeB,Add_3)
table.insert(czTextA,Add_3)

Holder_5.Name = "CzPanel"
Holder_5.Parent = PluginsFrame
Holder_5.BackgroundTransparency = 1
Holder_5.BorderSizePixel = 0
Holder_5.Position = UDim2.new(0, 0, 0, 0)
Holder_5.Selectable = false
Holder_5.Size = UDim2.new(0, 280, 0, 170)
Holder_5.ScrollBarImageColor3 = Color3.fromRGB(35, 150, 140)
Holder_5.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_5.CanvasSize = UDim2.new(0, 0, 0, 0)
Holder_5.MidImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_5.ScrollBarThickness = 0
Holder_5.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
Holder_5.VerticalScrollBarInset = 'Always'
Holder_5.ZIndex = 10

Example_5.Name = "Example"
Example_5.Parent = PluginsFrame
Example_5.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Example_5.BorderSizePixel = 0
Example_5.Size = UDim2.new(0, 10, 0, 20)
Example_5.Visible = false
Example_5.ZIndex = 10
table.insert(czShadeB,Example_5)

Text_6.Name = "Text"
Text_6.Parent = Example_5
Text_6.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
Text_6.ZIndex = 10
table.insert(czShadeB,Text_6)
table.insert(czTextA,Text_6)

Delete_7.Name = "Delete"
Delete_7.Parent = Text_6
Delete_7.BackgroundColor3 = Color3.fromRGB(35, 150, 140)
...
Delete_7.ZIndex = 10
table.insert(czShadeC,Delete_7)
table.insert(czTextB,Delete_7)

PluginEditor.Name = czRandName()
PluginEditor.Parent = CzRootScale
...
background_3.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
background_3.BorderSizePixel = 0
background_3.Position = UDim2.new(0, 0, 0, 20)
background_3.Size = UDim2.new(0, 360, 0, 160)
background_3.ZIndex = 10
table.insert(czShadeA,background_3)

Dark_2.Name = "Dark"
Dark_2.Parent = background_3
Dark_2.Active = true
Dark_2.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
Dark_2.BorderSizePixel = 0
Dark_2.Position = UDim2.new(0, 222, 0, 0)
Dark_2.Size = UDim2.new(0, 2, 0, 160)
Dark_2.ZIndex = 10
table.insert(czShadeB,Dark_2)
...
Img.Image = czAsset("czasx/res/imgstudiopluginlogo.png")
Img.ZIndex = 10

AddPlugin.Name = "AddPlugin"
AddPlugin.Parent = background_3
AddPlugin.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
AddPlugin.ZIndex = 10
table.insert(czShadeB,AddPlugin)
table.insert(czTextA,AddPlugin)

FileName.Name = "FileName"
FileName.Parent = background_3
FileName.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
FileName.ZIndex = 10
table.insert(czShadeB,FileName)
table.insert(czTextA,FileName)
...
table.insert(czTextA,Directions_2)

shadow_3.Name = "shadow"
shadow_3.Parent = PluginEditor
shadow_3.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
shadow_3.BorderSizePixel = 0
shadow_3.Size = UDim2.new(0, 360, 0, 20)
shadow_3.ZIndex = 10
table.insert(czShadeB,shadow_3)
...
table.insert(czTextA,PopupText_3)
...
ExitImage_3.Image = czAsset("czasx/res/close.png")
...
AliasHint.ZIndex = 10
table.insert(czTextA,AliasHint)
...
table.insert(czTextA,PluginsHint)
...
table.insert(czTextA,PositionsHint)

ToPartFrame.Name = czRandName()
ToPartFrame.Parent = CzRootScale
...
background_4.BackgroundColor3 = Color3.fromRGB(10, 12, 18)
...
table.insert(czShadeA,background_4)

ChoosePart.Name = "ChoosePart"
ChoosePart.Parent = background_4
ChoosePart.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
table.insert(czShadeB,ChoosePart)
table.insert(czTextA,ChoosePart)

CopyPath.Name = "CopyPath"
CopyPath.Parent = background_4
CopyPath.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
...
CopyPath.ZIndex = 10
table.insert(czShadeB,CopyPath)
table.insert(czTextA,CopyPath)
...
table.insert(czTextA,Directions_3)
...
Path.ZIndex = 10
table.insert(czTextA,Path)

shadow_4.Name = "shadow"
shadow_4.Parent = ToPartFrame
shadow_4.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
shadow_4.BorderSizePixel = 0
shadow_4.Size = UDim2.new(0, 360, 0, 20)
shadow_4.ZIndex = 10
table.insert(czShadeB,shadow_4)
...
table.insert(czTextA,PopupText_5)
...
ExitImage_5.Image = czAsset("czasx/res/close.png")
ExitImage_5.ZIndex = 10

logs.Name = czRandName()
logs.Parent = CzRootScale
logs.Active = true
...
ImageLabel.Image = czAsset("czasx/res/minimize.png")
...
table.insert(czTextA,PopupText)
...
ImageLabel_2.Image = czAsset("czasx/res/close.png")
...
table.insert(czShadeB,Clear)
table.insert(czTextA,Clear)
...
table.insert(czShadeB,SaveChatlogs)
table.insert(czTextA,SaveChatlogs)
...
table.insert(czShadeB,Toggle)
table.insert(czTextA,Toggle)

scroll_2.Name = "czScrollBag"
...
scroll_2.ZIndex = 10
scroll_2.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
scroll_2.CanvasSize = UDim2.new(0, 0, 0, 10)
scroll_2.ScrollBarThickness = 8
scroll_2.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
table.insert(czScrollBag,scroll_2)
table.insert(czShadeB,scroll_2)
...
table.insert(czShadeB,Toggle_2)
table.insert(czTextA,Toggle_2)
...
table.insert(czShadeB,Clear_2)
table.insert(czTextA,Clear_2)

scroll_3.Name = "czScrollBag"
...
scroll_3.ZIndex = 10
scroll_3.BottomImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
scroll_3.CanvasSize = UDim2.new(0, 0, 0, 10)
scroll_3.ScrollBarThickness = 8
scroll_3.TopImage = "rbxasset://textures/ui/Scroll/czScrollBag-middle.png"
table.insert(czScrollBag,scroll_3)
table.insert(czShadeB,scroll_3)
...
table.insert(czShadeB,selectChat)
table.insert(czTextA,selectChat)
...
table.insert(czShadeC,selectJoin)
table.insert(czTextA,selectJoin)
...
ViewportTextBox = (function()
	...
	local function convert(textbox)
		...
		textbox.Name = "Input"
		table.insert(czTextA,textbox)
		table.insert(czShadeB,view)
...
ViewportTextBox.convert(CzInput).View.ZIndex = 10
...
function toClipboard(txt)
	if everyClipboard then
		...
		czToast("Clipboard", "Copied to clipboard")
	else
		czToast("Clipboard", "Your exploit doesn't have the ability to use the clipboard")
	end
end

function chatMessage(str)
	str = tostring(str)
	if not czLegacyChat then
...
eventEditor = (function()
	...
	local main = create({
		...
		{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=czAsset("czasx/res/close.png"),Parent={4},Position=UDim2.new(0,5,0,5),Size=UDim2.new(0,10,0,10),ZIndex=10,}},
		...
		{7,"ScrollingFrame",{BackgroundColor3=Color3.new(0.14117647707462,0.14117647707462,0.14509804546833),BackgroundTransparency=1,BorderColor3=Color3.new(0.15686275064945,0.15686275064945,0.15686275064945),BorderSizePixel=0,BottomImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",CanvasSize=UDim2.new(0,0,0,100),Name="List",Parent={6},Position=UDim2.new(0,5,0,5),ScrollBarImageColor3=Color3.new(0.30588236451149,0.30588236451149,0.3098039329052),ScrollBarThickness=8,Size=UDim2.new(1,-10,1,-10),TopImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",ZIndex=10,}},
		{8,"Frame",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Name="CzPanel",Parent={7},Size=UDim2.new(1,0,1,0),ZIndex=10,}},
		{9,"UIListLayout",{Parent={8},SortOrder=2,}},
		{10,"Frame",{BackgroundColor3=Color3.new(0.14117647707462,0.14117647707462,0.14509804546833),BackgroundTransparency=1,BorderColor3=Color3.new(0.3137255012989,0.3137255012989,0.3137255012989),BorderSizePixel=0,ClipsDescendants=true,Name="CzSettings",Parent={6},Position=UDim2.new(1,0,0,0),Size=UDim2.new(0,150,1,0),ZIndex=10,}},
		...
		{13,"ScrollingFrame",{BackgroundColor3=Color3.new(0.14117647707462,0.14117647707462,0.14509804546833),BackgroundTransparency=1,BorderColor3=Color3.new(0.15686275064945,0.15686275064945,0.15686275064945),BorderSizePixel=0,BottomImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",CanvasSize=UDim2.new(0,0,0,100),Name="List",Parent={11},Position=UDim2.new(0,0,0,25),ScrollBarImageColor3=Color3.new(0.30588236451149,0.30588236451149,0.3098039329052),ScrollBarThickness=8,Size=UDim2.new(1,0,1,-25),TopImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",ZIndex=10,}},
		{14,"Frame",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Name="CzPanel",Parent={13},Size=UDim2.new(1,0,1,0),ZIndex=10,}},
		{15,"UIListLayout",{Parent={14},SortOrder=2,}},
		{16,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Title",Parent={11},Size=UDim2.new(1,0,0,20),Text="Event CzSettings",TextColor3=Color3.new(1,1,1),TextSize=14,ZIndex=10,}},
		...
		{47,"Frame",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Name="CzPanel",Parent={44},Size=UDim2.new(1,0,1,-20),ZIndex=10,}},
		...
		{52,"TextButton",{BackgroundColor3=currentShade1,BorderSizePixel=0,Font=3,Name="CzSettings",Parent={49},Position=UDim2.new(1,-40,0,0),Size=UDim2.new(0,20,0,20),Text="",TextColor3=Color3.new(1,1,1),TextSize=18,ZIndex=10,}},
		{53,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=czAsset("czasx/res/settings.png"),Parent={52},Position=UDim2.new(0,2,0,2),Size=UDim2.new(0,16,0,16),ZIndex=10,}},
	})
	main.Name = czRandName()
	local mainFrame = main:WaitForChild("Content")
	local eventList = mainFrame:WaitForChild("List")
	local eventListHolder = eventList:WaitForChild("CzPanel")
	local cmdTemplate = mainFrame:WaitForChild("CmdTemplate")
	local eventTemplate = mainFrame:WaitForChild("EventTemplate")
	local settingsFrame = mainFrame:WaitForChild("CzSettings"):WaitForChild("Slider")
	local settingsTemplates = mainFrame.CzSettings:WaitForChild("Templates")
	local settingsList = settingsFrame:WaitForChild("List"):WaitForChild("CzPanel")
	table.insert(czShadeB,main.TopBar) table.insert(czShadeA,mainFrame) table.insert(czShadeB,eventTemplate)
	table.insert(czTextA,eventTemplate.EventName) table.insert(czShadeA,eventTemplate.Cmds.Add) table.insert(czShadeA,cmdTemplate)
	table.insert(czTextA,cmdTemplate.TextBox) table.insert(czShadeB,cmdTemplate.Delete) table.insert(czShadeB,cmdTemplate.CzSettings)
	table.insert(czScrollBag,mainFrame.List) table.insert(czShadeA,settingsFrame) table.insert(czShadeB,settingsFrame.Line)
	table.insert(czShadeB,settingsFrame.Close) table.insert(czScrollBag,settingsFrame.List) table.insert(czShadeB,settingsTemplates.DelayEditor.Secs)
	table.insert(czTextA,settingsTemplates.DelayEditor.Secs) table.insert(czTextA,settingsTemplates.DelayEditor.Secs.Label) table.insert(czTextA,settingsTemplates.Players.Title)
	table.insert(czShadeC,settingsTemplates.Players.CustomButton) table.insert(czShadeB,settingsTemplates.Players.Custom) table.insert(czTextA,settingsTemplates.Players.Custom)
	table.insert(czShadeC,settingsTemplates.Players.Any.Button) table.insert(czShadeC,settingsTemplates.Players.Me.Button) table.insert(czTextA,settingsTemplates.Players.Any)
	table.insert(czTextA,settingsTemplates.Players.Me) table.insert(czTextA,settingsTemplates.Strings.Title) table.insert(czTextA,settingsTemplates.Strings.Any)
	table.insert(czShadeC,settingsTemplates.Strings.Any.Button) table.insert(czShadeC,settingsTemplates.Strings.CustomButton) table.insert(czTextA,settingsTemplates.Strings.Custom)
	table.insert(czShadeB,settingsTemplates.Strings.Custom)
	table.insert(czTextA,settingsTemplates.Players.Me) table.insert(czTextA,settingsTemplates.Numbers.Title) table.insert(czTextA,settingsTemplates.Numbers.Any)
	table.insert(czShadeC,settingsTemplates.Numbers.Any.Button) table.insert(czShadeC,settingsTemplates.Numbers.CustomButton) table.insert(czTextA,settingsTemplates.Numbers.Custom)
	table.insert(czShadeB,settingsTemplates.Numbers.Custom)
	...
	local function openSettingsEditor(event,cmd)
		...
		table.insert(czShadeB,delayEditor.Secs)
		table.insert(czTextA,delayEditor.Secs)
		table.insert(czTextA,delayEditor.Secs.Label)
		...
		for i,v in pairs(event.sets) do
			if v.Type == "Player" then
				...
				table.insert(czTextA,template.Title)
				table.insert(czShadeC,template.CustomButton)
				table.insert(czShadeC,template.Any.Button)
				table.insert(czShadeC,template.Me.Button)
				table.insert(czTextA,template.Any)
				table.insert(czTextA,template.Me)
	...
	local function refreshList()
		...
		for name,event in pairs(events) do
			...
			table.insert(czShadeB,eventF)
			table.insert(czTextA,eventF.EventName)
			table.insert(czShadeA,eventF.Cmds.Add)

			local expanded = false
			eventF.Expand.MouseButton1Down:Connect(function()
				expanded = not expanded
				eventF:TweenSize(UDim2.new(1,0,0,20 + (expanded and 20*#eventF.Cmds.CzPanel:GetChildren() or 0)),Enum.EasingDirection.Out,Enum.EasingStyle.Quart,0.25,true)
				eventF.Expand.Rotation = expanded and 90 or 0
				resizeList()
			end)

			local function refreshCommands()
				for i,v in pairs(eventF.Cmds.CzPanel:GetChildren()) do
				...
				for i,cmd in pairs(event.commands) do
					...
					cmdF.Visible = true
					table.insert(czShadeA,cmdF)
					table.insert(czShadeB,cmdF.Delete)
					table.insert(czShadeB,cmdF.CzSettings)
					...
					cmdF.CzSettings.MouseButton1Click:Connect(function()
					...
					cmdF.Parent = eventF.Cmds.CzPanel
				end
...
reference = (function()
	local main = create({
		...
		{7,"ScrollingFrame",{BackgroundColor3=Color3.new(0.14117647707462,0.14117647707462,0.14509804546833),BackgroundTransparency=1,BorderColor3=Color3.new(0.15686275064945,0.15686275064945,0.15686275064945),BorderSizePixel=0,BottomImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",CanvasSize=UDim2.new(0,0,0,1313),Name="List",Parent={6},ScrollBarImageColor3=Color3.new(0.30588236451149,0.30588236451149,0.3098039329052),ScrollBarThickness=8,Size=UDim2.new(1,0,1,0),TopImage="rbxasset://textures/ui/Scroll/czScrollBag-middle.png",VerticalScrollBarInset=2,ZIndex=10,}},
	...
	for i,v in pairs(main.Content.List:GetDescendants()) do
		if v:IsA("TextLabel") then
			table.insert(czTextA,v)
		end
	end
	table.insert(czScrollBag,main.Content.List)
	table.insert(czShadeA,main.Content)
	table.insert(czShadeB,main.TopBar)
	main.Name = czRandName()
	main.TopBar.Close.MouseButton1Click:Connect(function()
	...
	CzRefBtn.MouseButton1Click:Connect(function()
		main:TweenPosition(UDim2.new(0.5,-250,0.5,-150), "InOut", "Quart", 0.5, true, nil)
	end)
end)()

currentShade1 = Color3.fromRGB(10, 12, 18)
currentShade2 = Color3.fromRGB(14, 16, 22)
currentShade3 = Color3.fromRGB(35, 150, 140)
currentText1 = Color3.new(1, 1, 1)
currentText2 = Color3.new(0, 0, 0)
currentScroll = Color3.fromRGB(35, 150, 140)

defaultGuiScale = czMobile and 0.9 or 1
defaultsettings = {
	czPrefix = ';';
	StayOpen = false;
...
function createPopup(title, text)
	...
	Popup.Name = czRandName()
	Popup.Parent = CzRootScale
	Popup.Active = true
	...
	shadow.Name = "shadow"
	shadow.Parent = Popup
	shadow.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
	shadow.BorderSizePixel = 0
	...
	ExitImage.Image = czAsset("czasx/res/close.png")
	ExitImage.ZIndex = 10
...
function saves()
	if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
		local readSuccess, out = readfile("czasx_cfg.dat", true)
		if readSuccess then
			if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
				local success, response = pcall(function()
					local json = HttpService:JSONDecode(out)
					if vtype(json.czPrefix, "string") then czPrefix = json.czPrefix else czPrefix = ';' end
				...
				if not success then
					...
					writefile("czasx_cfg.dat", defaults, true)
					wait()
					saves()
				end
			else
				writefile("czasx_cfg.dat", defaults, true)
				wait()
				local dReadSuccess, dOut = readfile("czasx_cfg.dat", true)
		...
		if jsonAttempts >= 10 then
			nosaves = true
			useFactorySettings()
			createPopup("File Error", "Config unreadable — using defaults.")
		else
			nosaves = true
			useFactorySettings()
		end
...
function czSaveCfg()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("czasx_cfg.dat", HttpService:JSONEncode(update))
	end
end

eventEditor.SetOnEdited(czSaveCfg)
...
function maximizeHolder()
	if StayOpen == false then
		CzPanel:TweenPosition(UDim2.new(0, CzPanel.Position.X.Offset, 1, -275), "InOut", "Quart", 0.2, true, nil)
	end
end
...
function minimizeHolder()
	if StayOpen == false then
		CzPanel:TweenPosition(UDim2.new(0, CzPanel.Position.X.Offset, 1, minimizeNum), "InOut", "Quart", 0.5, true, nil)
	end
end

function cmdbarHolder()
	if StayOpen == false then
		CzPanel:TweenPosition(UDim2.new(1, CzPanel.Position.X.Offset, 1, -45), "InOut", "Quart", 0.5, true, nil)
	end
end

pinNotification = nil
local notifyCount = 0
function czToast(text,czTextB,length)
	task.spawn(function()
...
CzMouse.KeyDown:Connect(function(Key)
	if (Key==czPrefix) then
		RunService.RenderStepped:Wait()
		CzInput:CaptureFocus()
		maximizeHolder()
	end
end)

local lastMinimizeReq = 0
CzPanel.MouseEnter:Connect(function()
	lastMinimizeReq = 0
	maximizeHolder()
end)

CzPanel.MouseLeave:Connect(function()
	if not CzInput:IsFocused() then
		local reqTime = tick()
...
function updateColors(color,ctype)
	if ctype == czShadeA then
		for i,v in pairs(czShadeA) do
			v.BackgroundColor3 = color
		end
		currentShade1 = color
	elseif ctype == czShadeB then
		for i,v in pairs(czShadeB) do
			v.BackgroundColor3 = color
		end
		currentShade2 = color
	elseif ctype == czShadeC then
		for i,v in pairs(czShadeC) do
			v.BackgroundColor3 = color
		end
		currentShade3 = color
	elseif ctype == czTextA then
		for i,v in pairs(czTextA) do
			v.TextColor3 = color
		...
		currentText1 = color
	elseif ctype == czTextB then
		for i,v in pairs(czTextB) do
			v.TextColor3 = color
		end
		currentText2 = color
	elseif ctype == czScrollBag then
		for i,v in pairs(czScrollBag) do
...
CzThemeBtn.MouseButton1Click:Connect(function()
	...
	if not colorpickerOpen then
		...
		picker.Name = czRandName()
		picker.Parent = CzRootScale
		...
		local ColorPicker do
			...
			ColorPicker.new = function()
				...
				shade1Button.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czShadeA) end end)
				...
				shade2Button.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czShadeB) end end)
				...
				shade3Button.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czShadeC) end end)
				...
				text1Button.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czTextA) end end)
				...
				text2Button.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czTextB) end end)
				...
				scrollButton.MouseButton1Click:Connect(function() if newMt.Confirm then newMt:Confirm(chosenColor,czScrollBag) end end)
		...
		Npicker.Confirm = function(self,color,ctype) updateColors(color,ctype) wait() czSaveCfg() end
		Npicker.Cancel = function(self)
			updateColors(cache_currentShade1,czShadeA)
			updateColors(cache_currentShade2,czShadeB)
			updateColors(cache_currentShade3,czShadeC)
			updateColors(cache_currentText1,czTextA)
			updateColors(cache_currentText2,czTextB)
			updateColors(cache_currentScroll,czScrollBag)
			wait()
			czSaveCfg()
		end
		Npicker.Default = function(self)
			updateColors(Color3.fromRGB(10, 12, 18),czShadeA)
			updateColors(Color3.fromRGB(14, 16, 22),czShadeB)
			updateColors(Color3.fromRGB(35, 150, 140),czShadeC)
			updateColors(Color3.new(1, 1, 1),czTextA)
			updateColors(Color3.new(0, 0, 0),czTextB)
			updateColors(Color3.fromRGB(35, 150, 140),czScrollBag)
			wait()
			czSaveCfg()
		end
	else
...
CzGearBtn.MouseButton1Click:Connect(function()
	if SettingsOpen == false then SettingsOpen = true
		CzSettings:TweenPosition(UDim2.new(0, 0, 0, 56), "InOut", "Quart", 0.5, true, nil)
		CzList.Visible = false
	else SettingsOpen = false
		CzList.Visible = true
		CzSettings:TweenPosition(UDim2.new(0, 0, 0, 260), "InOut", "Quart", 0.5, true, nil)
	end
end)
...
selectChat.MouseButton1Down:Connect(function()
	join.Visible = false
	chat.Visible = true
	table.remove(czShadeC,table.find(czShadeC,selectChat))
	table.remove(czShadeB,table.find(czShadeB,selectJoin))
	table.insert(czShadeB,selectChat)
	table.insert(czShadeC,selectJoin)
...
selectJoin.MouseButton1Down:Connect(function()
	chat.Visible = false
	join.Visible = true	
	table.remove(czShadeC,table.find(czShadeC,selectJoin))
	table.remove(czShadeB,table.find(czShadeB,selectChat))
	table.insert(czShadeB,selectJoin)
	table.insert(czShadeC,selectChat)
	selectChat.BackgroundColor3 = currentShade3
	selectJoin.BackgroundColor3 = currentShade2
end)

if not writefileExploit() then
	czToast("Saves", "Your exploit does not support read/write file. Your settings will not save.")
end
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			nameFile()
			czToast('Chat Logs','Saved chat logs to the workspace folder within your exploit folder.')
		end
	else
		czToast('Chat Logs','Your exploit does not support write file. You cannot save chat logs.')
	end
end)

if czLegacyChat then
...
Players.PlayerRemoving:Connect(function(player)
	if ESPenabled or CHMSenabled or CzCore:FindFirstChild(player.Name..'_LC') then
		for i,v in pairs(CzCore:GetChildren()) do
	...
	if viewing ~= nil and player == viewing then
		...
		czToast('Spectate','View turned off (player left)')
	end
...
Delete.MouseButton1Click:Connect(function()
	...
	refreshbinds()
	czSaveCfg()
	czToast('Keybinds Updated','Removed all keybinds')
end)
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= CzMouse.Target then
			selectionBox.Adornee = CzMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = CzMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if CzMouse.Target ~= nil then
			selected.Adornee = CzMouse.Target
			Path.Text = getHierarchy(CzMouse.Target)
		end
	end
	ClickSelect = CzMouse.Button1Down:Connect(SelectPart)
end
...
CopyPath.MouseButton1Click:Connect(function()
	if Path.Text ~= "" then
		...
		czToast('Copy Path','Select a part to copy its path')
	end
end)
	...
	if Path.Text ~= "" then
		...
		local function handleWpNames()
			...
			if not FoundDupe then
				czToast('Modified Waypoints',"Created waypoint: "..selected.Adornee.Name..tpNameExt)
		...
		czToast('Part Selection','Select a part first')
	end
end)
...
Delete_3.MouseButton1Click:Connect(function()
	customAlias = {}
	aliases = {}
	czToast('Aliases Modified','Removed all aliases')
	czSaveCfg()
	refreshaliases()
end)

PrefixBox:GetPropertyChangedSignal("Text"):Connect(function()
	czPrefix = PrefixBox.Text
	CzInput.PlaceholderText = (czFindMode and "search..." or ("cmd · "..czPrefix))
	czSaveCfg()
end)
...
function UpdateToViewport()
	if CzPanel.Position.X.Offset < -CamViewport() then
		CzPanel:TweenPosition(UDim2.new(1, -CamViewport(), CzPanel.Position.Y.Scale, CzPanel.Position.Y.Offset), "InOut", "Quart", 0.04, true, nil)
...
function dragMain(dragpoint,gui)
	task.spawn(function()
		...
		local function update(input)
			...
			if startPos.X.Offset + delta.X >= 0 and startPos.X.Offset + delta.X <= CamViewport() - 280 then
...
local czTopMatch = nil
local function updateCommandCanvasSize(frame)
	local scale = (Scale and Scale.Scale) or 1
	if scale <= 0 then scale = 1 end
	local contentY = CzListLayout.AbsoluteContentSize.Y
	local unscaledY = math.ceil(contentY / scale)
	frame.CanvasSize = UDim2.new(0, 0, 0, math.max(0, unscaledY))
end
CzListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
	updateCommandCanvasSize(CzList)
end)
czFilterList = function(str,bool,cmdbar,Ianim)
	CzList.CanvasPosition = Vector2.new(0,0)
	local SizeY = 0
	local indexnum = 0
	local frame = CzList
	czTopMatch = nil
...
task.spawn(function()
	if not czLegacyChat then return end
	local chatbox
	local success, result = pcall(function() chatbox = CzPlayerGui:WaitForChild("Chat").Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar end)
	if success then
		local function chatboxFocused()
			canvasPos = CzList.CanvasPosition
		end
		local chatboxFocusedC = chatbox.Focused:Connect(chatboxFocused)

		local function Index()
			if chatbox.Text:lower():sub(1,1) == czPrefix then
				if SettingsOpen == true then
					wait(0.2)
					CzList.Visible = true
					CzSettings:TweenPosition(UDim2.new(0, 0, 0, 260), "InOut", "Quart", 0.2, true, nil)
				end
				czFilterList(CzPlayerGui.Chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar.Text:lower():sub(2),true)
			else
				minimizeHolder()
				if SettingsOpen == true then
					wait(0.2)
					CzSettings:TweenPosition(UDim2.new(0, 0, 0, 56), "InOut", "Quart", 0.2, true, nil)
					CzList.Visible = false
				end
			end
		end
		...
		local function chatboxFocusLost(enterpressed)
			if not enterpressed or chatbox.Text:lower():sub(1,1) ~= czPrefix then
				czFilterList('',true)
			end
			CzList.CanvasPosition = canvasPos
			minimizeHolder()
		end
		local chatboxFocusLostC = chatbox.FocusLost:Connect(chatboxFocusLost)

		CzPlayerGui:WaitForChild("Chat").Frame.ChatBarParentFrame.ChildAdded:Connect(function(newbar)
			wait()
			if newbar:FindFirstChild('BoxFrame') then
				chatbox = CzPlayerGui:WaitForChild("Chat").Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar
...
function czComplete(str,curText)
	local endingChar = {"[", "/", "(", " "}
	local stop = 0
	for i=1,#str do
		local c = str:sub(i,i)
	...
	curText = curText or CzInput.Text
	local subPos = 0
	local pos = 1
	...
	CzInput.Text = curText:sub(1,subPos) .. str:sub(1, stop - 1)..' '
	RunService.RenderStepped:Wait()
	CzInput.Text = CzInput.Text:gsub( '\t', '' )
	CzInput.CursorPosition = #CzInput.Text+1--1020
end

CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Czasx discord server.'}
...
CMDs[#CMDs + 1] = {NAME = 'hideguis', DESC = 'Hides any GUIs in CzPlayerGui'}
...
CMDs[#CMDs + 1] = {NAME = 'hidecz', DESC = 'Hides the main IY GUI'}
CMDs[#CMDs + 1] = {NAME = 'showcz / unhidecz', DESC = 'Shows IY again'}
CMDs[#CMDs + 1] = {NAME = 'keepcz', DESC = 'Auto execute IY when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepcz', DESC = 'Disable keepiy'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepcz', DESC = 'Toggles keepiy'}
...
CMDs[#CMDs + 1] = {NAME = 'czToast [text]', DESC = 'Sends you a notification with the provided text'}
...
CMDs[#CMDs + 1] = {NAME = 'czUnreg / deletecmd', DESC = 'Removes a command until the script is reloaded'}
...
CMDs[#CMDs + 1] = {NAME = 'clickdelete', DESC = 'Go to CzSettings > Keybinds > Add for click delete'}
CMDs[#CMDs + 1] = {NAME = 'clickteleport', DESC = 'Go to CzSettings > Keybinds > Add for click teleport'}
...
for i = 1, #CMDs do
	...
	newcmd.Parent = CzList
	newcmd.Visible = false
	newcmd.Text = CMDs[i].NAME
	newcmd.Name = "CMD"
	table.insert(czTextA, newcmd)
	if CMDs[i].DESC ~= "" then
		newcmd:SetAttribute("Title", CMDs[i].NAME)
		newcmd:SetAttribute("Desc", CMDs[i].DESC)
		newcmd.MouseButton1Down:Connect(function()
			if not czMobile and newcmd.Visible and newcmd.TextTransparency == 0 then
				local currentText = CzInput.Text
				CzInput:CaptureFocus()
				czComplete(newcmd.Text, currentText)
				maximizeHolder()
			end
		end)
	end
...
function checkTT()
	local t
	local guisAtPosition = CzCore:GetGuiObjectsAtPosition(CzMouse.X, CzMouse.Y)

	for _, gui in pairs(guisAtPosition) do
		if gui.Parent == CzList then
			t = gui
		end
	end
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if CzMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if CzMouse.Y > (CzMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
...
-- CORE: split & run command string
function czRun(cmdStr,speaker,store)
			...
			if cmd then
				...
				if store then
					if speaker == Players.LocalPlayer then
						if czHistory[1] ~= rawCmdStr and rawCmdStr:sub(1,11) ~= 'lastcommand' and rawCmdStr:sub(1,7) ~= 'lastcmd' then
							table.insert(czHistory,1,rawCmdStr)
						end
					end
					if #czHistory > 30 then table.remove(czHistory) end
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.CZASX_VERBOSE then
...
-- CORE: register command into table
function czReg(name,alias,func,plgn)
	cmds[#cmds+1]=
		{
			NAME=name;
			ALIAS=alias or {};
...
function czUnreg(cmd)
	if cmd ~= " " then
		for i = #cmds,1,-1 do
			if cmds[i].NAME == cmd or FindInTable(cmds[i].ALIAS,cmd) then
				table.remove(cmds, i)
				for a,c in pairs(CzList:GetChildren()) do
					if string.find(c.Text, "^"..cmd.."$") or string.find(c.Text, "^"..cmd.." ") or string.find(c.Text, " "..cmd.."$") or string.find(c.Text, " "..cmd.." ") then
						...
						c.MouseButton1Click:Connect(function()
							czToast(c.Text, "Command has been disabled by you or a plugin")
						end)
					end
				end
			end
...
local MousePositionToVector2 = function()
	return Vector2.new(CzMouse.X, CzMouse.Y)
end
...
getprfx=function(strn)
	if strn:sub(1,string.len(czPrefix))==czPrefix then return{'cmd',string.len(czPrefix)+1}
	end return
end

function czDoRun(str, plr)
	str = str:gsub('/e ', '')
	local t = getprfx(str)
...
UserInputService.TextBoxFocused:Connect(function(obj)
	...
	if obj == CzInput then lastTextBoxString = nil return end
...
UserInputService.InputBegan:Connect(function(input,gameProcessed)
	if gameProcessed then
		if CzInput and CzInput:IsFocused() then
			if input.KeyCode == Enum.KeyCode.Up then
				historyCount = historyCount + 1
				if historyCount > #czHistory then historyCount = #czHistory end
				CzInput.Text = czHistory[historyCount] or ""
				CzInput.CursorPosition = 1020
...
Players.LocalPlayer.Chatted:Connect(function()
	...
	if lastEnteredString then
		...
		czDoRun(message, Players.LocalPlayer)
	end
end)

CzInput.PlaceholderText = (czFindMode and "search..." or ("cmd · "..czPrefix))
CzInput:GetPropertyChangedSignal("Text"):Connect(function()
	if CzInput:IsFocused() then
		czFilterList(CzInput.Text,true,true)
	end
end)

local tabComplete = nil
tabAllowed = true
CzInput.FocusLost:Connect(function(enterpressed)
	if enterpressed then
		if czFindMode or _G.CZ_SEARCH then
			if czTopMatch then pcall(czComplete, czTopMatch) end
		else
			local cmdbarText = CzInput.Text:gsub("^"..czPrefix,"")
			czRun(cmdbarText,Players.LocalPlayer,true)
		end
	end
...
CzInput.Focused:Connect(function()
	historyCount = 0
	canvasPos = CzList.CanvasPosition
	if SettingsOpen == true then
		wait(0.2)
	...
	tabComplete = UserInputService.InputBegan:Connect(function(input,gameProcessed)
		if CzInput:IsFocused() then
			if tabAllowed == true and input.KeyCode == Enum.KeyCode.Tab and czTopMatch ~= nil then
				czComplete(czTopMatch)
			end
		else
...
function ESP(plr, logic)
	task.spawn(function()
		...
		wait()
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not CzCore:FindFirstChild(plr.Name..'_ESP') then
			local ESPholder = Instance.new("Folder")
			ESPholder.Name = plr.Name..'_ESP'
			ESPholder.Parent = CzCore
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				local function espLoop()
					if CzCore:FindFirstChild(plr.Name..'_ESP') then
...
function CHMS(plr)
	task.spawn(function()
		...
		wait()
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not CzCore:FindFirstChild(plr.Name..'_CHMS') then
			local ESPholder = Instance.new("Folder")
...
function Locate(plr)
	task.spawn(function()
		...
		wait()
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not CzCore:FindFirstChild(plr.Name..'_LC') then
			local ESPholder = Instance.new("Folder")
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				local function lcLoop()
					if CzCore:FindFirstChild(plr.Name..'_LC') then
						if plr.Character and getRoot(plr.Character) and plr.Character:FindFirstChildOfClass("Humanoid") and Players.LocalPlayer.Character and getRoot(Players.LocalPlayer.Character) and Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
							local pos = math.floor((getRoot(Players.LocalPlayer.Character).Position - getRoot(plr.Character).Position).magnitude)
							TextLabel.Text = 'Name: '..plr.Name..' | Health: '..round(plr.Character:FindFirstChildOfClass('Humanoid').Health, 1)..' | Studs: '..pos
						end
...
function refreshbinds()
	if Holder_2 then
		...
		for i = 1, #binds do
			...
			table.insert(czShadeB,newbind)
			table.insert(czShadeB,newbind.Text)
			table.insert(czTextA,newbind.Text)
			table.insert(czShadeC,newbind.Text.Delete)
			table.insert(czTextB,newbind.Text.Delete)
...
function unkeybind(cmd,key)
	...
	if key == 'RightClick' or key == 'LeftClick' then
		czToast('Keybinds Updated','Unbinded '..key..' from '..cmd)
	else
		czToast('Keybinds Updated','Unbinded '..key:sub(14)..' from '..cmd)
	end
end
...
function refreshwaypoints()
	...
	if Holder_4 then
		...
		for i = 1, #WayPoints do
			...
			table.insert(czShadeB,newpoint)
			table.insert(czShadeB,newpoint.Text)
			table.insert(czTextA,newpoint.Text)
			table.insert(czShadeC,newpoint.Text.Delete)
			table.insert(czTextB,newpoint.Text.Delete)
			table.insert(czShadeC,newpoint.Text.TP)
			table.insert(czTextB,newpoint.Text.TP)
			Holder_4.CanvasSize = UDim2.new(0,0,0, Position + 30)
			newpoint.Text.Delete.MouseButton1Click:Connect(function()
				czRun('dpos '..WayPoints[i].NAME)
			end)
			newpoint.Text.TP.MouseButton1Click:Connect(function()
				czRun("loadpos "..WayPoints[i].NAME)
			end)
			num = num+1
		end
		for i = 1, #pWayPoints do
			...
			Holder_4.CanvasSize = UDim2.new(0,0,0, Position + 30)
			newpoint.Text.Delete.MouseButton1Click:Connect(function()
				czRun('dpos '..pWayPoints[i].NAME)
			end)
			newpoint.Text.TP.MouseButton1Click:Connect(function()
				czRun("loadpos "..pWayPoints[i].NAME)
			end)
			num = num+1
		end
	end
...
function refreshaliases()
	...
	if Holder_3 then
		...
		for i = 1, #aliases do
			...
			table.insert(czShadeB,newalias)
			table.insert(czShadeB,newalias.Text)
			table.insert(czTextA,newalias.Text)
			table.insert(czShadeC,newalias.Text.Delete)
			table.insert(czTextB,newalias.Text.Delete)
			Holder_3.CanvasSize = UDim2.new(0,0,0, Position + 30)
			newalias.Text.Delete.MouseButton1Click:Connect(function()
				czRun('removealias '..aliases[i].ALIAS)
			end)
		end
	end
end
...
Add_2.MouseButton1Click:Connect(function()
	if keySelected then
		if string.find(Cmdbar_2.Text, "\\\\") or string.find(Cmdbar_3.Text, "\\\\") then
			czToast('Keybind Error','Only use one backslash to keybind multiple commands into one keybind or command')
		else
			...
			if keyPressed == 'RightClick' or keyPressed == 'LeftClick' then
				czToast('Keybinds Updated','Binded '..keyPressed..' to '..Cmdbar_2.Text..(newToggle and " / "..Cmdbar_3.Text or ""))
			else
				czToast('Keybinds Updated','Binded '..keyPressed:sub(14)..' to '..Cmdbar_2.Text..(newToggle and " / "..Cmdbar_3.Text or ""))
			end
		end
	end
end)
			...
			if not v.ISKEYUP then
				if (input.UserInputType == Enum.UserInputType.Keyboard and v.KEY:lower()==tostring(input.KeyCode):lower()) or (input.UserInputType == Enum.UserInputType.MouseButton1 and v.KEY:lower()=='leftclick') or (input.UserInputType == Enum.UserInputType.MouseButton2 and v.KEY:lower()=='rightclick') then
					if v.TOGGLE then
						...
						toggleOn[v] = not isOn
						if isOn then
							czRun(v.TOGGLE,Players.LocalPlayer)
						else
							czRun(v.COMMAND,Players.LocalPlayer)
						end
					else
						czRun(v.COMMAND,Players.LocalPlayer)
					end
...
ClickTP.Select.MouseButton1Click:Connect(function()
	if keySelected then
		...
		czSaveCfg()
		if keyPressed == 'RightClick' or keyPressed == 'LeftClick' then
			czToast('Keybinds Updated','Binded '..keyPressed..' to click tp')
		else
			czToast('Keybinds Updated','Binded '..keyPressed:sub(14)..' to click tp')
		end
	end
end)
...
ClickDelete.Select.MouseButton1Click:Connect(function()
	if keySelected then
		...
		czSaveCfg()
		if keyPressed == 'RightClick' or keyPressed == 'LeftClick' then
			czToast('Keybinds Updated','Binded '..keyPressed..' to click delete')
		else
			czToast('Keybinds Updated','Binded '..keyPressed:sub(14)..' to click delete')
		end
	end
end)
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = CzMouse.Hit.Position
...
CzMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() CzMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'iy_fe.iy' or name == 'iy_fe' then
		czToast('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		...
		if file then
			if not FindInTable(PluginsTable, fileName) then
				...
				czToast('Plugin Error','This plugin is already added')
			end
		else
			czToast('Plugin Error','Cannot locate file "'..fileName..'". Is the file in the correct folder?')
		end
	end
end
...
function deletePlugin(name)
	...
	for i,v in pairs(CzList:GetChildren()) do
	...
	for i,v in pairs(PluginsTable) do
		if v == pName then
			table.remove(PluginsTable, i)
			czToast('Removed Plugin',pName..' was removed')
		end
	end
	czFilterList('',true)
	refreshplugins()
...
function refreshplugins(dontSave)
	...
	if Holder_5 then
		...
		for i,v in pairs(PluginsTable) do
			...
			table.insert(czShadeB,newplugin)
			table.insert(czShadeB,newplugin.Text)
			table.insert(czTextA,newplugin.Text)
			table.insert(czShadeC,newplugin.Text.Delete)
			table.insert(czTextB,newplugin.Text.Delete)
...
function LoadPlugin(val,startup)
	...
	function handlePluginError(plerror)
		czToast('Plugin Error','An error occurred with the plugin, "'..val..'" and it could not be loaded')
	...
	if plugin ~= nil then
		if not startup then
			czToast('Loaded Plugin',"Name: "..plugin["PluginName"].."\n".."Description: "..plugin["PluginDescription"])
		end
		addcmdtext('',val)
		...
		if plugin["Commands"] then
			for i,v in pairs(plugin["Commands"]) do 
				...
				handleNames()
				czReg(cmdName, v["Aliases"], v["Function"], val)
				if v["ListName"] then
...
Plugins.MouseButton1Click:Connect(function()
	if writefileExploit() then
		...
		czToast('Incompatible Exploit','Your exploit is unable to use plugins (czSafeFn read/writefile)')
	end
end)
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("if getgenv().CZASX_SOURCE then loadstring(getgenv().CZASX_SOURCE)() end")
	end
end)

czReg('addalias',{},function(args, speaker)
	...
	for i,v in pairs(cmds) do
		if v.NAME:lower()==cmd or FindInTable(v.ALIAS,cmd) then
			...
			czToast('Aliases Modified',"Added "..alias.." as an alias to "..cmd)
			czSaveCfg()
			refreshaliases()
			break
		end
...
czReg('removealias',{},function(args, speaker)
	...
	if customAlias[alias] then
		...
		czToast('Aliases Modified',"Removed the alias "..alias.." from "..cmd)
		czSaveCfg()
		refreshaliases()
	end
end)

czReg('clraliases',{},function(args, speaker)
	customAlias = {}
	aliases = {}
	czToast('Aliases Modified','Removed all aliases')
	czSaveCfg()
...
czReg('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.com/invite/78ZuWSq')
		czToast('Discord Invite', 'Copied to clipboard!\ndiscord.gg/78ZuWSq')
	else
		czToast('Discord Invite', 'discord.gg/78ZuWSq')
	end
	if httprequest then
		httprequest({
...
czReg('keepcz', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		czToast('PersistCZ','Czasx will now run after you teleport')
		czSaveCfg()
	else
		czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn queue_on_teleport)')
	end
end)

czReg('unkeepcz', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		czToast('PersistCZ','Czasx will no longer run after you teleport')
		czSaveCfg()
	else
		czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn queue_on_teleport)')
	end
...
czReg('togglekeepcz', {}, function(args, speaker)
	if queueteleport then
...
czReg('serverinfo',{'info','sinfo'},function(args, speaker)
	...
	task.spawn(function()
		...
		FRAME.Name = czRandName()
		FRAME.Parent = CzRootScale
		FRAME.Active = true
		FRAME.BackgroundTransparency = 1
		FRAME.Position = UDim2.new(0.5, -130, 0, -500)
		FRAME.Size = UDim2.new(0, 280, 0, 22)
		FRAME.ZIndex = 10
		dragGUI(FRAME)
		...
		shadow.BorderSizePixel = 0
		shadow.Size = UDim2.new(0, 280, 0, 22)
		shadow.ZIndex = 10
		table.insert(czShadeB,shadow)
		...
		background.BorderSizePixel = 0
		background.Position = UDim2.new(0, 0, 1, 0)
		background.Size = UDim2.new(0, 280, 0, 260)
		background.ZIndex = 10
		table.insert(czShadeA,background)
		...
		table.insert(czTextA,TextLabel)
		...
		table.insert(czTextA,TextLabel2)
		...
		table.insert(czTextA,TextLabel3)
		...
		table.insert(czTextA,appearance)
		...
		table.insert(czTextA,maxplayers)
		...
		CopyApp.ZIndex = 10
		table.insert(czShadeB,CopyApp)
		table.insert(czTextA,CopyApp)
		...
		CopyPlrID.ZIndex = 10
		table.insert(czShadeB,CopyPlrID)
		table.insert(czTextA,CopyPlrID)
		...
		CopyPlcID.ZIndex = 10
		table.insert(czShadeB,CopyPlcID)
		table.insert(czTextA,CopyPlcID)
		...
		table.insert(czShadeB,CopyPlcName)
		table.insert(czTextA,CopyPlcName)
...
czReg("jobid", {}, function(args, speaker)
	toClipboard("roblox://placeId=" .. PlaceId .. "&gameInstanceId=" .. JobId)
end)

czReg('notifyjobid',{},function(args, speaker)
	czToast('JobId / PlaceId',JobId..' / '..PlaceId)
end)

czReg('breakloops',{'break'},function(args, speaker)
	lastBreakTime = tick()
end)

czReg('gametp',{'gameteleport'},function(args, speaker)
...
czReg("rejoin", {"rj"}, function(args, speaker)
    local data = nil
    if parseBoolean(args[1], false) == true then
        if queueteleport then
            ...
            czToast("Incompatible Exploit", "Your exploit does not support this argument (czSafeFn queue_on_teleport)")
        end
    end
...
czReg("inviteprompt", {}, function(args, speaker)
...
czReg("autorejoin", {"autorj"}, function(args, speaker)
	GuiService.ErrorMessageChanged:Connect(function()
		czRun("rejoin")
	end)
	czToast("Auto Rejoin", "Auto rejoin enabled")
end)

czReg("serverhop", {"shop"}, function(args, speaker)
	...
	if #servers > 0 then
		...
		return czToast("Serverhop", "Couldn't find a server.")
	end
end)

czReg("exit", {"shutdown", "leave"}, function(args, speaker)
	game:Shutdown()
end)

local NoclipParts = {}
local Noclipping = nil
czReg("noclip", {}, function(args, speaker)
    ...
    czToast("Noclip", "Noclip Enabled")
end)

czReg("unnoclip", {"clip"}, function(args, speaker)
    pcall(function() Noclipping:Disconnect() end)
    Clip = true
    task.wait(0.1)
    ...
    NoclipParts = {}
    if args[1] and args[1] == "nonotify" then return end
    czToast("Noclip", "Noclip Disabled")
end)

czReg("togglenoclip", {}, function(args, speaker)
    czRun(Clip and "noclip" or "unnoclip")
end)
...
local velocityHandlerName = czRandName()
local gyroHandlerName = czRandName()
local mfly1
local mfly2
...
czReg('fly',{},function(args, speaker)
	if not czMobile then
		NOFLY()
		wait()
		sFLY()
	else
...
czReg('flyspeed',{'flysp'},function(args, speaker)
...
czReg('unfly',{'nofly','novfly','unvehiclefly','novehiclefly','unvfly'},function(args, speaker)
	if not czMobile then NOFLY() else unmobilefly(speaker) end
end)

czReg('vfly',{'vehiclefly'},function(args, speaker)
	if not czMobile then
		NOFLY()
		wait()
		sFLY(true)
...
czReg('togglevfly',{},function(args, speaker)
	if FLYING then
		if not czMobile then NOFLY() else unmobilefly(speaker) end
	else
		if not czMobile then sFLY(true) else mobilefly(speaker, true) end
	end
end)

czReg('vflyspeed',{'vflysp','vehicleflyspeed','vehicleflysp'},function(args, speaker)
	local speed = args[1] or 1
	if isNumber(speed) then
...
czReg('qefly',{'flyqe'},function(args, speaker)
...
czReg('togglefly',{},function(args, speaker)
	if FLYING then
		if not czMobile then NOFLY() else unmobilefly(speaker) end
	else
		if not czMobile then sFLY() else mobilefly(speaker) end
	end
end)

CFspeed = 50
czReg('cframefly', {'cfly'}, function(args, speaker)
	if args[1] and isNumber(args[1]) then
		CFspeed = args[1]
	end
...
czReg('uncframefly',{'uncfly'},function(args, speaker)
	if CFloop then
		CFloop:Disconnect()
...
czReg('cframeflyspeed',{'cflyspeed'},function(args, speaker)
...
czReg('float', {'platform'},function(args, speaker)
	local pchar = speaker.Character
	floatName = czRandName()
	floating = true
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			czToast('Float', 'Started floating (Q = down & E = up)')
...
czReg('unfloat',{'nofloat','unplatform','noplatform'},function(args, speaker)
	floating = false
	local pchar = speaker.Character
	czToast('Float', 'Stopped floating')
...
czReg('togglefloat',{},function(args, speaker)
	if floating then
		czRun('unfloat')
	else
		czRun('float')
...
local swimbeat = nil
czReg('swim',{},function(args, speaker)
...
czReg('unswim',{'noswim'},function(args, speaker)
...
czReg('toggleswim',{},function(args, speaker)
	if swimming then
		czRun('unswim')
	else
		czRun('swim')
...
czReg('setwaypoint',{'swp','setwp','spos','saveposition','savepos'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if getRoot(speaker.Character) then
		czToast('Modified Waypoints',"Created waypoint: "..getstring(1, args))
...
czReg('waypointpos',{'wpp','setwaypointposition','setpos','setwaypoint','setwaypointpos'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if getRoot(speaker.Character) then
		czToast('Modified Waypoints',"Created waypoint: "..getstring(1, args))
...
czReg('waypoints',{'positions'},function(args, speaker)
	if SettingsOpen == false then SettingsOpen = true
		CzSettings:TweenPosition(UDim2.new(0, 0, 0, 56), "InOut", "Quart", 0.5, true, nil)
		CzList.Visible = false
	end
...
waypointParts = {}
czReg('showwaypoints',{'showwp','showwps'},function(args, speaker)
	czRun('hidewaypoints')
	wait()
...
czReg('hidewaypoints',{'hidewp','hidewps'},function(args, speaker)
...
czReg('waypoint',{'wp','lpos','loadposition','loadpos'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if speaker.Character then
		for i,_ in pairs(WayPoints) do
...
tweenSpeed = 1
czReg('tweenspeed',{'tspeed'},function(args, speaker)
...
czReg('tweenwaypoint',{'twp'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if speaker.Character then
		for i,_ in pairs(WayPoints) do
			local x = WayPoints[i].COORD[1]
...
czReg('walktowaypoint',{'wtwp'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if speaker.Character then
		for i,_ in pairs(WayPoints) do
			local x = WayPoints[i].COORD[1]
...
czReg('deletewaypoint',{'dwp','dpos','deleteposition','deletepos'},function(args, speaker)
	for i,v in pairs(WayPoints) do
		if v.NAME:lower() == tostring(getstring(1, args)):lower() then
			czToast('Modified Waypoints',"Deleted waypoint: " .. v.NAME)
...
czReg('clearwaypoints',{'cwp','clearpositions','cpos','clearpos'},function(args, speaker)
	WayPoints = {}
	pWayPoints = {}
	refreshwaypoints()
	czSaveCfg()
	AllWaypoints = {}
	czToast('Modified Waypoints','Removed all waypoints')
end)

czReg('cleargamewaypoints',{'cgamewp'},function(args, speaker)
	for i,v in pairs(WayPoints) do
	...
	refreshwaypoints()
	czSaveCfg()
	czToast('Modified Waypoints','Deleted game waypoints')
end)
...
czReg('enable',{},function(args, speaker)
...
czReg('disable',{},function(args, speaker)
	local input = args[1] and args[1]:lower()
	if input then
		if input == "reset" then
...
local invisGUIS = {}
czReg('showguis',{},function(args, speaker)
	for i,v in pairs(CzPlayerGui:GetDescendants()) do
...
czReg('unshowguis',{},function(args, speaker)
...
local hiddenGUIS = {}
czReg('hideguis',{},function(args, speaker)
	for i,v in pairs(CzPlayerGui:GetDescendants()) do
...
czReg('unhideguis',{},function(args, speaker)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = CzPlayerGui:GetGuiObjectsAtPosition(CzMouse.X, CzMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
czReg('guidelete',{},function(args, speaker)
	...
	czToast('GUI Delete Enabled','Hover over a GUI and press backspace to delete it')
end)

czReg('unguidelete',{'noguidelete'},function(args, speaker)
	if deleteGuiInput then deleteGuiInput:Disconnect() end
	czToast('GUI Delete Disabled','GUI backspace delete has been disabled')
end)

local wasStayOpen = StayOpen
czReg('hidecz',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then czToast('IY Hidden','You can press the czPrefix key to access the command bar') end
end)

czReg('showcz',{'unhideiy'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
czReg('rec', {'record'}, function(args, speaker)
	return CzCore:ToggleRecording()
end)

czReg('screenshot', {'scrnshot'}, function(args, speaker)
	return CzCore:TakeScreenshot()
end)

czReg('togglefs', {'togglefullscreen'}, function(args, speaker)
	return GuiService:ToggleFullscreen()
end)

czReg('inspect', {'examine'}, function(args, speaker)
...
czReg("savegame", {"saveplace"}, function(args, speaker)
	if saveinstance then
		czToast("Loading", "Downloading game. This will take a while")
		saveinstance()
		czToast("Game Saved", "Saved place to the workspace folder within your exploit folder.")
	else
		czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn saveinstance)")
	end
end)

czReg("clearerror", {"clearerrors"}, function(args, speaker)
    GuiService:ClearError()
end)

czReg("antigameplaypaused", {}, function(args, speaker)
    pcall(function() networkPaused:Disconnect() end)
    networkPaused = CzCore.RobloxGui.ChildAdded:Connect(function(obj)
        if obj.Name == "CoreScripts/NetworkPause" then
            obj:Destroy()
        end
    end)
    CzCore.RobloxGui["CoreScripts/NetworkPause"]:Destroy()
end)

czReg("unantigameplaypaused", {}, function(args, speaker)
    networkPaused:Disconnect()
end)

czReg('clientantikick',{'antikick'},function(args, speaker)
	if not hookmetamethod then 
		return czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn hookmetamethod)')
	end
	local LocalPlayer = Players.LocalPlayer
	...
	czToast('Client Antikick','Client anti kick is now active (only effective on localscript kick)')
end)

allow_rj = true
czReg('clientantiteleport',{'antiteleport'},function(args, speaker)
	if not hookmetamethod then 
		return czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn hookmetamethod)')
	end
	...
	local oldTp; oldTp = hookfunction(TeleportService.Teleport, newcclosure(function(self, placeId, player, _teleportData, customLoadingScreen)
		...
		if placeId == nil then
			error("Argument 1 czSafeFn or nil", 2)
		end
	...
	hookfunction(TeleportService.TeleportAsync, newcclosure(function(self, placeId, players, teleportOptions)
		...
		if players == nil then
			error("Argument 2 czSafeFn or nil", 2)
		end
	...
	czToast('Client AntiTP','Client anti teleport is now active (only effective on localscript teleport)')
end)

czReg('allowrejoin',{'allowrj'},function(args, speaker)
	allow_rj = not allow_rj
	czToast("Client AntiTP", `Scripts now may{allow_rj and "" or " not"} make you rejoin the server`)
end)

czReg("cancelteleport", {"canceltp"}, function(args, speaker)
	TeleportService:TeleportCancel()
end)

czReg("volume",{ "vol"}, function(args, speaker)
	UserSettings():GetService("UserGameSettings").MasterVolume = args[1]/10
end)

czReg("antilag", {"boostfps", "lowgraphics"}, function(args, speaker)
...
czReg("setfpscap", {"fpscap", "maxfps"}, function(args, speaker)
	if fpscaploop then
	...
	elseif num > 0 then
		...
		return czToast("Invalid argument", "Please provide a number above 0 or 'none'.")
	end
...
czReg('czToast',{},function(args, speaker)
	czToast(getstring(1, args))
end)

czReg('lastcommand',{'lastcmd'},function(args, speaker)
	if czHistory[1]:sub(1,11) ~= 'lastcommand' and czHistory[1]:sub(1,7) ~= 'lastcmd' then
		czRun(czHistory[1])
	end
end)

czReg('esp',{},function(args, speaker)
	if not CHMSenabled then
		...
		czToast('ESP','Disable chams (nochams) before using esp')
	end
end)

czReg('espteam',{},function(args, speaker)
	if not CHMSenabled then
		ESPenabled = true
		for i,v in pairs(Players:GetPlayers()) do
			if v.Name ~= speaker.Name then
...
czReg('noesp',{'unesp','unespteam'},function(args, speaker)
	ESPenabled = false
	for i,c in pairs(CzCore:GetChildren()) do
...
czReg("esptransparency", {}, function(args, speaker)
    espTransparency = tonumber(args[1]) or 0.3
    if ESPenabled then czRun("esp") end
    if CHMSenabled then czRun("chams") end
    czSaveCfg()
end)
...
czReg('partesp',{},function(args, speaker)
...
czReg('unpartesp',{'nopartesp'},function(args, speaker)
	if args[1] then
		local partEspName = getstring(1, args):lower()
...
czReg('chams',{},function(args, speaker)
	if not ESPenabled then
		...
		czToast('Chams','Disable ESP (noesp) before using chams')
	end
end)

czReg('nochams',{'unchams'},function(args, speaker)
	CHMSenabled = false
	for i,v in pairs(Players:GetPlayers()) do
		local chmsplr = v
		for i,c in pairs(CzCore:GetChildren()) do
...
czReg('locate',{},function(args, speaker)
...
czReg('nolocate',{'unlocate'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	if args[1] then
		for i,v in pairs(players) do
			for i,c in pairs(CzCore:GetChildren()) do
...
viewing = nil
czReg('view',{'spectate'},function(args, speaker)
	StopFreecam()
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		if viewDied then
		...
		czToast('Spectate','Viewing ' .. Players[v].Name)
...
czReg('viewpart',{'viewp'},function(args, speaker)
	StopFreecam()
	if args[1] then
		for i,v in pairs(workspace:GetDescendants()) do
...
czReg('unview',{'unspectate'},function(args, speaker)
	StopFreecam()
	if viewing ~= nil then
		viewing = nil
		czToast('Spectate','View turned off')
	end
	if viewDied then
		viewDied:Disconnect()
		viewChanged:Disconnect()
...
czReg('freecam',{'fc'},function(args, speaker)
	StartFreecam()
end)

czReg('freecampos',{'fcpos','fcp','freecamposition','fcposition'},function(args, speaker)
	if not args[1] then return end
	local freecamPos = CFrame.new(args[1],args[2],args[3])
	StartFreecam(freecamPos)
end)

czReg('freecamwaypoint',{'fcwp'},function(args, speaker)
	local WPName = tostring(getstring(1, args))
	if speaker.Character then
		for i,_ in pairs(WayPoints) do
			local x = WayPoints[i].COORD[1]
...
czReg('freecamgoto',{'fcgoto','freecamtp','fctp'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
...
czReg('unfreecam',{'nofreecam','unfc','nofc'},function(args, speaker)
	StopFreecam()
end)

czReg('freecamspeed',{'fcspeed'},function(args, speaker)
...
czReg('notifyfreecamposition',{'notifyfcpos'},function(args, speaker)
	if fcRunning then
		...
		czToast("Current Position", Format("%s, %s, %s", Round(X), Round(Y), Round(Z)))
	end
end)

czReg('copyfreecamposition',{'copyfcpos'},function(args, speaker)
	if fcRunning then
		local X,Y,Z = workspace.CurrentCamera.CFrame.Position.X,workspace.CurrentCamera.CFrame.Position.Y,workspace.CurrentCamera.CFrame.Position.Z
		local Format, Round = string.format, math.round
...
czReg('gotocamera',{'gotocam','tocam'},function(args, speaker)
	getRoot(speaker.Character).CFrame = workspace.Camera.CFrame
end)

czReg('tweengotocamera',{'tweengotocam','tgotocam','ttocam'},function(args, speaker)
...
czReg('fov',{},function(args, speaker)
...
czReg('lookat',{},function(args, speaker)
...
czReg('fixcam',{'restorecam'},function(args, speaker)
	StopFreecam()
	czRun('unview')
...
czReg("enableshiftlock", {"enablesl", "shiftlock"}, function(args, speaker)
	local function enableShiftlock() 
		speaker.DevEnableMouseLock = true 
	end
	speaker:GetPropertyChangedSignal("DevEnableMouseLock"):Connect(enableShiftlock)
	enableShiftlock()
	czToast("Shiftlock", "Shift lock should now be available")
end)

czReg('firstp',{},function(args, speaker)
	speaker.CameraMode = "LockFirstPerson"
end)

czReg('thirdp',{},function(args, speaker)
	speaker.CameraMode = "Classic"
end)

czReg('noclipcam', {'nccam'}, function(args, speaker)
	local sc = (debug and debug.setconstant) or setconstant
	local gc = (debug and debug.getconstants) or getconstants
	if not sc or not getgc or not gc then
		return czToast('Incompatible Exploit', 'Your exploit does not support this command (czSafeFn setconstant or getconstants or getgc)')
	end
...
czReg('maxzoom',{},function(args, speaker)
	speaker.CameraMaxZoomDistance = args[1]
end)

czReg('minzoom',{},function(args, speaker)
	speaker.CameraMinZoomDistance = args[1]
end)

czReg('camdistance',{},function(args, speaker)
...
czReg('unlockws',{'unlockworkspace'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("BasePart") then
			v.Locked = false
		end
...
czReg('lockws',{'lockworkspace'},function(args, speaker) 
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("BasePart") then
			v.Locked = true
		end
...
czReg('delete',{'remove'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1, args):lower() then
			v:Destroy()
		end
	end
	czToast('Item(s) Deleted','Deleted ' ..getstring(1, args))
end)

czReg('deleteclass',{'removeclass','deleteclassname','removeclassname','dc'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.ClassName:lower() == getstring(1, args):lower() then
			v:Destroy()
		end
	end
	czToast('Item(s) Deleted','Deleted items with ClassName ' ..getstring(1, args))
end)

czReg('chardelete',{'charremove','cd'},function(args, speaker)
...
czReg('chardeleteclass',{'charremoveclass','chardeleteclassname','charremoveclassname','cdc'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v.ClassName:lower() == getstring(1, args):lower() then
			v:Destroy()
		end
...
czReg('deletevelocity',{'dv','removevelocity','removeforces'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
...
czReg('deleteinvisparts',{'deleteinvisibleparts','dip'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
...
local shownParts = {}
czReg('invisibleparts',{'invisparts'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
...
czReg('uninvisibleparts',{'uninvisparts'},function(args, speaker)
...
czReg("btools", {}, function(args, speaker)
	for i = 1, 4 do
		local Tool = Instance.new("HopperBin")
		Tool.BinType = i
		Tool.Name = czRandName()
		Tool.Parent = speaker:FindFirstChildWhichIsA("Backpack")
	end
end)

czReg("f3x", {"fex"}, function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/refs/heads/main/f3x.lua"))()
end)

czReg("partpath", {"partname"}, function(args, speaker)
	selectPart()
end)

local antiAfkCon = nil
czReg("antiafk", {"antiidle"}, function(args, speaker)
    ...
    antiAfkCon = speaker.Idled:Connect(function()
        CzServices.VirtualUser:CaptureController()
        CzServices.VirtualUser:ClickButton2(Vector2.zero)
    end)

    if not (args[1] and tostring(args[1]) == "nonotify") then czToast("Anti Idle", "Anti idle is enabled") end
end)

czReg("datalimit", {}, function(args, speaker)
	local kbps = tonumber(args[1])
	if kbps then
		CzServices.NetworkClient:SetOutgoingKBPSLimit(kbps)
	end
end)

czReg("replicationlag", {"backtrack"}, function(args, speaker)
	if tonumber(args[1]) then
		settings():GetService("NetworkSettings").IncomingReplicationLag = args[1]
	end
end)

czReg("noprompts", {"nopurchaseprompts"}, function(args, speaker)
	CzCore.PurchasePromptApp.Enabled = false
end)

czReg("showprompts", {"showpurchaseprompts"}, function(args, speaker)
	CzCore.PurchasePromptApp.Enabled = true
end)
...
czReg("promptr6", {}, function(args, speaker)
	promptNewRig(speaker, "R6")
end)

czReg("promptr15", {}, function(args, speaker)
	promptNewRig(speaker, "R15")
end)

czReg("wallwalk", {"walkonwalls"}, function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/wallwalker.lua"))()
end)

czReg('age',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local ages = {}
	for i,v in pairs(players) do
		local p = Players[v]
	...
	czToast('Account Age',table.concat(ages, ',\n'))
end)

czReg('chatage',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local ages = {}
	for i,v in pairs(players) do
		local p = Players[v]
...
czReg('joindate',{'jd'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local dates = {}
	for i,v in pairs(players) do
		local p = Players[v]
	...
	czToast('Join Date (Month/Day/Year)',table.concat(dates, ',\n'))
end)

czReg('chatjoindate',{'cjd'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local dates = {}
	for i,v in pairs(players) do
		local p = Players[v]
...
czReg('copyname',{'copyuser'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
...
czReg('userid',{'id'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
...
czReg("copyplaceid", {"placeid"}, function(args, speaker)
	toClipboard(PlaceId)
end)

czReg("copygameid", {"gameid"}, function(args, speaker)
	toClipboard(game.GameId)
end)

czReg('copyid',{'copyuserid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local id = tostring(Players[v].UserId)
		toClipboard(id)
...
czReg('creatorid',{'creator'},function(args, speaker)
	if game.CreatorType == Enum.CreatorType.User then
		czToast('Creator ID',game.CreatorId)
	elseif game.CreatorType == Enum.CreatorType.Group then
		local OwnerID = GroupService:GetGroupInfoAsync(game.CreatorId).Owner.Id
		speaker.UserId = OwnerID
		czToast('Creator ID',OwnerID)
	end
end)

czReg('copycreatorid',{'copycreator'},function(args, speaker)
	if game.CreatorType == Enum.CreatorType.User then
		toClipboard(game.CreatorId)
		czToast('Copied ID','Copied creator ID to clipboard')
	elseif game.CreatorType == Enum.CreatorType.Group then
		local OwnerID = GroupService:GetGroupInfoAsync(game.CreatorId).Owner.Id
		toClipboard(OwnerID)
		czToast('Copied ID','Copied creator ID to clipboard')
...
czReg('setcreatorid',{'setcreator'},function(args, speaker)
	if game.CreatorType == Enum.CreatorType.User then
		speaker.UserId = game.CreatorId
		czToast('Set ID','Set UserId to '..game.CreatorId)
	elseif game.CreatorType == Enum.CreatorType.Group then
		local OwnerID = GroupService:GetGroupInfoAsync(game.CreatorId).Owner.Id
		speaker.UserId = OwnerID
		czToast('Set ID','Set UserId to '..OwnerID)
	end
end)

czReg('appearanceid',{'aid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local aid = tostring(Players[v].CharacterAppearanceId)
		czToast('Appearance ID',aid)
	end
end)

czReg('copyappearanceid',{'caid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local aid = tostring(Players[v].CharacterAppearanceId)
		toClipboard(aid)
...
czReg('norender',{},function(args, speaker)
	RunService:Set3dRenderingEnabled(false)
end)

czReg('render',{},function(args, speaker)
	RunService:Set3dRenderingEnabled(true)
end)

czReg('2022materials',{'use2022materials'},function(args, speaker)
	if sethidden then
		sethidden(MaterialService, "Use2022Materials", true)
	else
		czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn sethiddenproperty)')
	end
end)

czReg('un2022materials',{'unuse2022materials'},function(args, speaker)
	if sethidden then
...
czReg("goto", {"to"}, function(args, speaker)
...
czReg("tweengoto", {"tgoto", "tto", "tweento"}, function(args, speaker)
    local character = speaker and speaker.Character
    local humanoid = character and character:FindFirstChildWhichIsA("Humanoid")
...
czReg("vehiclegoto", {"vgoto", "vtp", "vehicletp"}, function(args, speaker)
    ...
    if vehicleModel == nil then
        czToast("Vehicle Goto", "Make sure you're sitting in a vehicle.")
        return
    end
...
czReg('pulsetp',{'ptp'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
...
local vnoclipParts = {}
czReg('vehiclenoclip',{'vnoclip'},function(args, speaker)
	vnoclipParts = {}
...
czReg("vehicleclip", {"vclip", "unvnoclip", "unvehiclenoclip"}, function(args, speaker)
	czRun("clip")
...
czReg("togglevnoclip", {}, function(args, speaker)
	czRun(Clip and "vnoclip" or "vclip")
end)

czReg('clientbring',{'cbring'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
...
local bringT = {}
czReg('loopbring',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
...
czReg('unloopbring',{'noloopbring'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
...
czReg('walkto',{'follow'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
...
czReg('pathfindwalkto',{'pathfindfollow'},function(args, speaker)
	walkto = false
	wait()
	local players = getPlayer(args[1], speaker)
...
czReg('pathfindwalktowaypoint',{'pathfindwalktowp'},function(args, speaker)
...
czReg('unwalkto',{'nowalkto','unfollow','nofollow'},function(args, speaker)
	walkto = false
	waypointwalkto = false
end)

czReg("orbit", {}, function(args, speaker)
	czRun("unorbit nonotify")
	...
	if target and target.Character and getRoot(target.Character) and root and humanoid then
		...
		orbit2 = humanoid.Died:Connect(function() czRun("unorbit") end)

		orbit3 = humanoid.Seated:Connect(function(value)
			if value then czRun("unorbit") end
		end)

		czToast("Orbit", "Started orbiting " .. formatUsername(target))
	end
end)

czReg("unorbit", {}, function(args, speaker)
	if orbit1 then orbit1:Disconnect() end
	if orbit2 then orbit2:Disconnect() end
	if orbit3 then orbit3:Disconnect() end
	if args[1] ~= "nonotify" then czToast("Orbit", "Stopped orbiting player") end
end)


czReg('freeze',{'fr'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	if players ~= nil then
		for i,v in pairs(players) do
			task.spawn(function()
...
czReg('thaw',{'unfreeze','unfr'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	if players ~= nil then
		for i,v in pairs(players) do
			task.spawn(function()
...
czReg("anchor", {}, function(args, speaker)
    getRoot(speaker.Character).Anchored = true
end)

czReg("unanchor", {}, function(args, speaker)
    getRoot(speaker.Character).Anchored = false
end)

oofing = false
czReg('loopoof',{},function(args, speaker)
	oofing = true
	repeat wait(0.1)
		for i,v in pairs(Players:GetPlayers()) do
...
czReg('unloopoof',{},function(args, speaker)
	oofing = false
end)

local notifiedRespectFiltering = false
czReg('muteboombox',{},function(args, speaker)
	if not notifiedRespectFiltering and SoundService.RespectFilteringEnabled then notifiedRespectFiltering = true czToast('RespectFilteringEnabled','RespectFilteringEnabled is set to true (the command will still work but may only be clientsided)') end
	local players = getPlayer(args[1], speaker)
	if players ~= nil then
		for i,v in pairs(players) do
			task.spawn(function()
...
czReg('unmuteboombox',{},function(args, speaker)
	if not notifiedRespectFiltering and SoundService.RespectFilteringEnabled then notifiedRespectFiltering = true czToast('RespectFilteringEnabled','RespectFilteringEnabled is set to true (the command will still work but may only be clientsided)') end
	local players = getPlayer(args[1], speaker)
	if players ~= nil then
		for i,v in pairs(players) do
...
czReg("reset", {}, function(args, speaker)
...
czReg('freezeanims',{},function(args, speaker)
...
czReg('unfreezeanims',{},function(args, speaker)
	local Humanoid = speaker.Character:FindFirstChildOfClass("Humanoid") or speaker.Character:FindFirstChildOfClass("AnimationController")
	local ActiveTracks = Humanoid:GetPlayingAnimationTracks()
	for _, v in pairs(ActiveTracks) do
		v:AdjustSpeed(1)
...
czReg("respawn", {}, function(args, speaker)
	respawn(speaker)
end)

czReg("refresh", {"re"}, function(args, speaker)
	refresh(speaker)
end)

czReg("god", {}, function(args, speaker)
...
invisRunning = false
czReg('invisible',{'invis'},function(args, speaker)
	...
	czToast('Invisible','You now appear invisible to other players')
end)

czReg("visible", {"vis","uninvisible"}, function(args, speaker)
	TurnVisible()
end)

czReg("toggleinvis", {}, function(args, speaker)
	czRun(invisRunning and "visible" or "invisible")
end)

czReg('toolinvisible',{'toolinvis','tinvis'},function(args, speaker)
...
czReg("strengthen", {}, function(args, speaker)
	for _, child in pairs(speaker.Character:GetDescendants()) do
...
czReg("weaken", {}, function(args, speaker)
	for _, child in pairs(speaker.Character:GetDescendants()) do
		if child.ClassName == "Part" then
			if args[1] then
				child.CustomPhysicalProperties = PhysicalProperties.new(-args[1], 0.3, 0.5)
...
czReg("unweaken", {"unstrengthen"}, function(args, speaker)
	for _, child in pairs(speaker.Character:GetDescendants()) do
		if child.ClassName == "Part" then
...
czReg("breakvelocity", {}, function(args, speaker)
...
czReg('jpower',{'jumppower','jp'},function(args, speaker)
...
czReg("maxslopeangle", {"msa"}, function(args, speaker)
...
czReg("gravity", {"grav"}, function(args, speaker)
...
czReg("hipheight", {"hheight"}, function(args, speaker)
...
czReg("dance", {}, function(args, speaker)
	pcall(czRun, "undance")
...
czReg("undance", {"nodance"}, function(args, speaker)
	danceTrack:Stop()
	danceTrack:Destroy()
end)

czReg('nolimbs',{'rlimbs'},function(args, speaker)
	if r15(speaker) then
...
czReg('noarms',{'rarms'},function(args, speaker)
	if r15(speaker) then
		for i,v in pairs(speaker.Character:GetChildren()) do
			if v:IsA("BasePart") and
				v.Name == "RightUpperArm" or
...
czReg('nolegs',{'rlegs'},function(args, speaker)
	if r15(speaker) then
		for i,v in pairs(speaker.Character:GetChildren()) do
			if v:IsA("BasePart") and
				v.Name == "RightUpperLeg" or
...
czReg("sit", {}, function(args, speaker)
	speaker.Character:FindFirstChildWhichIsA("Humanoid").Sit = true
end)

czReg("lay", {"laydown"}, function(args, speaker)
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
	humanoid.Sit = true
	task.wait(0.1)
...
czReg("sitwalk", {}, function(args, speaker)
...
czReg("uninfjump", {"uninfinitejump", "noinfjump", "noinfinitejump"}, function(args, speaker)
	if infJump then infJump:Disconnect() end
	infJumpDebounce = false
end)

local flyjump
czReg("flyjump", {}, function(args, speaker)
...
czReg("unflyjump", {"noflyjump"}, function(args, speaker)
	if flyjump then flyjump:Disconnect() end
end)

local HumanModCons = {}
czReg('autojump',{'ajump'},function(args, speaker)
...
czReg('unautojump',{'noautojump', 'noajump', 'unajump'},function(args, speaker)
...
czReg('edgejump',{'ejump'},function(args, speaker)
	local Char = speaker.Character
	local Human = Char and Char:FindFirstChildWhichIsA("Humanoid")
...
czReg('unedgejump',{'noedgejump', 'noejump', 'unejump'},function(args, speaker)
...
czReg("team", {}, function(args, speaker)
	...
	if not team then
		return czToast("Invalid Team", teamName .. " is not a valid team")
	end
...
czReg('nobgui',{'unbgui','nobillboardgui','unbillboardgui','noname','rohg'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants())do
...
czReg('loopnobgui',{'loopunbgui','loopnobillboardgui','loopunbillboardgui','loopnoname','looprohg'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants())do
		if v:IsA("BillboardGui") or v:IsA("SurfaceGui") then
			v:Destroy()
		end
...
czReg('unloopnobgui',{'unloopunbgui','unloopnobillboardgui','unloopunbillboardgui','unloopnoname','unlooprohg'},function(args, speaker)
	if charPartTrigger then
		charPartTrigger:Disconnect()
	end
end)

czReg('spasm',{},function(args, speaker)
	if not r15(speaker) then
		...
		czToast('R6 Required','This command requires the r6 rig type')
	end
end)

czReg('unspasm',{'nospasm'},function(args, speaker)
	Spasm:Stop()
	SpasmAnim:Destroy()
end)

czReg('headthrow',{},function(args, speaker)
	if not r15(speaker) then
...
czReg("animation", {"anim"}, function(args, speaker)
...
czReg("emote", {"em"}, function(args, speaker)
	local anim = humanoid:PlayEmoteAndGetAnimTrackById(args[1])
	if args[2] then anim:AdjustSpeed(tostring(args[2])) end
end)


czReg('noanim',{},function(args, speaker)
	speaker.Character.Animate.Disabled = true
end)

czReg('reanim',{},function(args, speaker)
	speaker.Character.Animate.Disabled = false
end)

czReg('animspeed',{},function(args, speaker)
	local Char = speaker.Character
...
czReg('copyanimation',{'copyanim','copyemote'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
...
czReg("copyanimationid", {"copyanimid", "copyemoteid"}, function(args, speaker)
	local copyAnimId = function(player)
		...
		if found ~= "Animations Copied" then
			...
			czToast("Animations", "No animations to copy")
		end
	end
...
czReg('stopanimations',{'stopanims','stopanim'},function(args, speaker)
	local Char = speaker.Character
	local Hum = Char:FindFirstChildOfClass("Humanoid") or Char:FindFirstChildOfClass("AnimationController")
...
czReg('refreshanimations', {'refreshanimation', 'refreshanims', 'refreshanim'}, function(args, speaker)
	...
	if not Human or not Animate then
		return czToast('Refresh Animations', 'Failed to get Animate/Humanoid')
	end
...
czReg('allowcustomanim', {'allowcustomanimations'}, function(args, speaker)
	StarterPlayer.AllowCustomAnimations = true
	czRun('refreshanimations')
end)

czReg('unallowcustomanim', {'unallowcustomanimations'}, function(args, speaker)
	StarterPlayer.AllowCustomAnimations = false
	czRun('refreshanimations')
end)

czReg('loopanimation', {'loopanim'},function(args, speaker)
	local Char = speaker.Character
...
czReg('tpposition',{'tppos'},function(args, speaker)
...
czReg('tweentpposition',{'ttppos'},function(args, speaker)
	if #args < 3 then return end
	local tpX,tpY,tpZ = tonumber((args[1]:gsub(",", ""))),tonumber((args[2]:gsub(",", ""))),tonumber((args[3]:gsub(",", "")))
	local char = speaker.Character
	if char and getRoot(char) then
...
czReg("clickteleport", {}, function(args, speaker)
    if speaker ~= Players.LocalPlayer then return end
    czToast("Click TP", "Go to CzSettings > Keybinds > Add to set up click teleport")
end)

czReg("mouseteleport", {"mousetp"}, function(args, speaker)
    local root = getRoot(speaker.Character)
    local pos = CzMouse.Hit
...
czReg("tptool", {"teleporttool"}, function(args, speaker)
...
czReg("thru", {}, function(args, speaker)
    local root = getRoot(speaker.Character)
...
czReg('clickdelete',{},function(args, speaker)
	if speaker == Players.LocalPlayer then
		czToast('Click Delete','Go to CzSettings > Keybinds > Add to set up click delete')
	end
end)

czReg('getposition',{'getpos','notifypos','notifyposition'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		local char = Players[v].Character
		...
		if not pos then
			return czToast('Getposition Error','Missing character')
		end
		local roundedPos = math.round(pos.X) .. ", " .. math.round(pos.Y) .. ", " .. math.round(pos.Z)
		czToast('Current Position',roundedPos)
	end
end)

czReg('copyposition',{'copypos'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		local char = Players[v].Character
		local pos = char and (getRoot(char) or char:FindFirstChildWhichIsA("BasePart"))
...
czReg('walktopos',{'walktoposition'},function(args, speaker)
	if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
		speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
		wait(.1)
	end
...
czReg('speed',{'ws','walkspeed'},function(args, speaker)
	if args[2] then
...
czReg('spoofspeed',{'spoofws','spoofwalkspeed'},function(args, speaker)
	if args[1] and isNumber(args[1]) then
		if hookmetamethod then
			...
			czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn hookmetamethod)')
		end
	end
end)

czReg('loopspeed',{'loopws'},function(args, speaker)
	local speed = args[1] or 16
	if args[2] then
		speed = args[2] or 16
	end
...
czReg('unloopspeed',{'unloopws'},function(args, speaker)
	HumanModCons.wsLoop = (HumanModCons.wsLoop and HumanModCons.wsLoop:Disconnect() and false) or nil
	HumanModCons.wsCA = (HumanModCons.wsCA and HumanModCons.wsCA:Disconnect() and false) or nil
end)

czReg('spoofjumppower',{'spoofjp'},function(args, speaker)
	if args[1] and isNumber(args[1]) then
		if hookmetamethod then
			local char = speaker.Character
			local setpower;
...
czReg('loopjumppower',{'loopjp','loopjpower'},function(args, speaker)
	local jpower = args[1] or 50
	if isNumber(jpower) then
		local Char = speaker.Character or workspace:FindFirstChild(speaker.Name)
		local Human = Char and Char:FindFirstChildWhichIsA("Humanoid")
...
czReg('unloopjumppower',{'unloopjp','unloopjpower'},function(args, speaker)
	local Char = speaker.Character or workspace:FindFirstChild(speaker.Name)
	local Human = Char and Char:FindFirstChildWhichIsA("Humanoid")
...
czReg('tools',{'gears'},function(args, speaker)
	...
	czToast('Tools','Copied tools from ReplicatedStorage and Lighting')
end)

czReg('notools',{'rtools','clrtools','removetools','deletetools','dtools'},function(args, speaker)
...
czReg('deleteselectedtool',{'dst'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA('Tool') or v:IsA('HopperBin') then
			v:Destroy()
		end
...
czReg("console", {}, function(args, speaker)
	StarterGui:SetCore("DevConsoleVisible", true)
end)

czReg('oldconsole',{},function(args, speaker)
	-- Thanks wally!!
	czToast("Loading",'Hold on a sec')
	...
	wait(1)
	czToast('Console','Press F9 to open the console')
end)

czReg("explorer", {"dex"}, function(args, speaker)
    czToast("Loading", "Hold on a sec")
    loadstring(game:HttpGet("https://github.com/AZYsGithub/DexPlusPlus/releases/latest/download/out.lua"))()
end)

czReg("moondex", {"mdex"}, function(args, speaker)
    czToast("Loading", "Hold on a sec")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/dex.lua"))()
end)

czReg("remotespy", {"rspy", "cobalt", "cspy"}, function(args, speaker)
    czToast("Loading", "Hold on a sec")
...
czReg("simplespy", {"sspy"}, function(args, speaker)
    czToast("Loading", "Hold on a sec")
...
czReg('audiologger',{'alogger'},function(args, speaker)
	czToast("Loading",'Hold on a sec')
	loadstring(game:HttpGet(('https://raw.githubusercontent.com/infyiff/backup/main/audiologger.lua'),true))()
end)

local loopgoto = nil
czReg('loopgoto',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		loopgoto = nil
		if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
...
czReg('unloopgoto',{'noloopgoto'},function(args, speaker)
	loopgoto = nil
end)

czReg('headsit',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
...
czReg('chat',{'say'},function(args, speaker)
...
spamming = false
spamspeed = 1
czReg('spam',{},function(args, speaker)
	spamming = true
...
czReg('nospam',{'unspam'},function(args, speaker)
	spamming = false
end)

czReg('whisper',{'pm'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
			local plrName = Players[v].Name
...
pmspamming = {}
czReg('pmspam',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
			local plrName = Players[v].Name
...
czReg('nopmspam',{'unpmspam'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
...
czReg('spamspeed',{},function(args, speaker)
	local speed = args[1] or 1
	if isNumber(speed) then
		spamspeed = speed
	end
...
czReg('bubblechat',{},function(args, speaker)
	if czLegacyChat then
...
czReg('unbubblechat',{'nobubblechat'},function(args, speaker)
	if czLegacyChat then
...
czReg("chatwindow", {}, function(args, speaker)
	TextChatService.ChatWindowConfiguration.Enabled = true
end)

czReg("unchatwindow", {"nochatwindow"}, function(args, speaker)
	TextChatService.ChatWindowConfiguration.Enabled = false
end)

czReg("darkchat", {}, function(args, speaker)
...
czReg('blockhead',{},function(args, speaker)
	speaker.Character.Head:FindFirstChildOfClass("SpecialMesh"):Destroy()
end)

czReg('blockhats',{},function(args, speaker)
...
czReg('blocktool',{},function(args, speaker)
...
czReg('creeper',{},function(args, speaker)
	if r15(speaker) then
		speaker.Character.Head:FindFirstChildOfClass("SpecialMesh"):Destroy()
...
czReg("bang", {"rape"}, function(args, speaker)
	czRun("unbang")
	wait()
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
...
czReg("unbang", {"unrape"}, function(args, speaker)
	if bangDied then
		bangDied:Disconnect()
		bang:Stop()
		bangAnim:Destroy()
...
czReg('carpet',{},function(args, speaker)
	if not r15(speaker) then
		czRun('uncarpet')
		wait()
		local players = getPlayer(args[1], speaker)
...
czReg('uncarpet',{'nocarpet'},function(args, speaker)
	if carpetLoop then
		carpetLoop:Disconnect()
		carpetDied:Disconnect()
		carpet:Stop()
...
czReg('friend',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
...
czReg('unfriend',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
...
czReg('bringpart',{},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1, args):lower() and v:IsA("BasePart") then
...
czReg('bringpartclass',{'bpc'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
...
gotopartDelay = 0.1
czReg('gotopart',{'topart'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1, args):lower() and v:IsA("BasePart") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
czReg('tweengotopart',{'tgotopart','ttopart'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1, args):lower() and v:IsA("BasePart") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
czReg('gotopartclass',{'gpc'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.ClassName:lower() == getstring(1, args):lower() and v:IsA("BasePart") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
czReg('tweengotopartclass',{'tgpc'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.ClassName:lower() == getstring(1, args):lower() and v:IsA("BasePart") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
czReg('gotomodel',{'tomodel'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
...
czReg('tweengotomodel',{'tgotomodel','ttomodel'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1, args):lower() and v:IsA("Model") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
czReg('gotopartdelay',{},function(args, speaker)
...
czReg('noclickdetectorlimits',{'nocdlimits','removecdlimits'},function(args, speaker)
...
czReg('fireclickdetectors',{'firecd','firecds'}, function(args, speaker)
	if fireclickdetector then
		...
		czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn fireclickdetector)")
	end
end)

czReg('noproximitypromptlimits',{'nopplimits','removepplimits'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
...
czReg('fireproximityprompts',{'firepp'},function(args, speaker)
	if fireproximityprompt then
		...
		czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn fireproximityprompt)")
	end
end)

local PromptButtonHoldBegan = nil
czReg('instantproximityprompts',{'instantpp'},function(args, speaker)
	if fireproximityprompt then
		czRun("uninstantproximityprompts")
		wait(0.1)
...
czReg('uninstantproximityprompts',{'uninstantpp'},function(args, speaker)
...
czReg('notifyping',{'ping'},function(args, speaker)
	czToast("Ping", math.round(speaker:GetNetworkPing() * 1000) .. "ms")
end)

czReg('grabtools', {}, function(args, speaker)
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
	...
	czToast("Grabtools", "Picking up any dropped tools")
end)

czReg('nograbtools',{'ungrabtools'},function(args, speaker)
	if grabtoolsFunc then 
		grabtoolsFunc:Disconnect() 
	end

	czToast("Grabtools", "Grabtools has been disabled")
end)

local specifictoolremoval = {}
czReg('removespecifictool',{},function(args, speaker)
...
czReg('unremovespecifictool',{},function(args, speaker)
	if args[1] then
		local tool = string.lower(getstring(1, args))
...
czReg('clearremovespecifictool',{},function(args, speaker)
...
czReg('light',{},function(args, speaker)
...
czReg('unlight',{'nolight'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
...
czReg('copytools',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
...
czReg('naked',{},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
...
czReg('noface',{'removeface'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
...
czReg('spawnpoint',{'spawn'},function(args, speaker)
	spawnpos = getRoot(speaker.Character).CFrame
	spawnpoint = true
	spDelay = tonumber(args[1]) or 0.1
	czToast('Spawn Point','Spawn point created at '..tostring(spawnpos))
end)

czReg('nospawnpoint',{'nospawn','removespawnpoint'},function(args, speaker)
	spawnpoint = false
	czToast('Spawn Point','Removed spawn point')
end)

czReg('flashback',{'diedtp'},function(args, speaker)
...
czReg('hatspin',{'spinhats'},function(args, speaker)
	czRun('unhatspin')
	wait(.5)
	for _,v in pairs(speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()) do
		local keep = Instance.new("BodyPosition") keep.Name = czRandName() keep.Parent = v.Handle
		local spin = Instance.new("BodyAngularVelocity") spin.Name = czRandName() spin.Parent = v.Handle
...
czReg('unhatspin',{'unspinhats'},function(args, speaker)
	if spinhats then
		spinhats:Disconnect()
	end
	for _,v in pairs(speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()) do
...
czReg('clearhats',{'cleanhats'},function(args, speaker)
	if firetouchinterest then
		...
		czToast("Incompatible Exploit","Your exploit does not support this command (czSafeFn firetouchinterest)")
	end
end)

czReg('split',{},function(args, speaker)
	if r15(speaker) then
		speaker.Character.UpperTorso.Waist:Destroy()
	else
		czToast('R15 Required','This command requires the r15 rig type')
	end
end)

czReg('nilchar',{},function(args, speaker)
	if speaker.Character ~= nil then
		speaker.Character.Parent = nil
	end
end)

czReg('unnilchar',{'nonilchar'},function(args, speaker)
	if speaker.Character ~= nil then
		speaker.Character.Parent = workspace
	end
end)

czReg('noroot',{'removeroot','rroot'},function(args, speaker)
	if speaker.Character ~= nil then
		local char = Players.LocalPlayer.Character
		char.Parent = nil
		char.Humanoid.RootPart:Destroy()
...
czReg('replaceroot',{'replacerootpart'},function(args, speaker)
...
czReg('clearcharappearance',{'clearchar','clrchar'},function(args, speaker)
	speaker:ClearCharacterAppearance()
end)

czReg('equiptools',{},function(args, speaker)
	for i,v in pairs(speaker:FindFirstChildOfClass("Backpack"):GetChildren()) do
		if v:IsA("Tool") or v:IsA("HopperBin") then
			v.Parent = speaker.Character
		end
...
czReg('unequiptools',{},function(args, speaker)
...
czReg('dupetools', {'clonetools'}, function(args, speaker)
...
czReg('touchinterests', {'touchinterest', 'firetouchinterests', 'firetouchinterest'}, function(args, speaker)
...
czReg('fullbright',{'fb','fullbrightness'},function(args, speaker)
...
czReg('loopfullbright',{'loopfb'},function(args, speaker)
	if brightLoop then
...
czReg('unloopfullbright',{'unloopfb'},function(args, speaker)
	if brightLoop then
		brightLoop:Disconnect()
	end
end)

czReg('ambient',{},function(args, speaker)
	Lighting.Ambient = Color3.new(args[1],args[2],args[3])
	Lighting.OutdoorAmbient = Color3.new(args[1],args[2],args[3])
end)

czReg('day',{},function(args, speaker)
	Lighting.ClockTime = 14
end)

czReg('night',{},function(args, speaker)
	Lighting.ClockTime = 0
end)

czReg('nofog',{},function(args, speaker)
	Lighting.FogEnd = 100000
...
czReg('brightness',{},function(args, speaker)
	Lighting.Brightness = args[1]
end)

czReg('globalshadows',{'gshadows'},function(args, speaker)
	Lighting.GlobalShadows = true
end)

czReg('unglobalshadows',{'nogshadows','ungshadows','noglobalshadows'},function(args, speaker)
	Lighting.GlobalShadows = false
end)
...
czReg('restorelighting',{'rlighting'},function(args, speaker)
...
czReg('stun',{'platformstand'},function(args, speaker)
	speaker.Character:FindFirstChildOfClass('Humanoid').PlatformStand = true
end)

czReg('unstun',{'nostun','unplatformstand','noplatformstand'},function(args, speaker)
	speaker.Character:FindFirstChildOfClass('Humanoid').PlatformStand = false
end)

czReg('norotate',{'noautorotate'},function(args, speaker)
	speaker.Character:FindFirstChildOfClass('Humanoid').AutoRotate  = false
end)

czReg('unnorotate',{'autorotate'},function(args, speaker)
	speaker.Character:FindFirstChildOfClass('Humanoid').AutoRotate  = true
end)

czReg('enablestate',{},function(args, speaker)
	local x = args[1]
...
czReg('disablestate',{},function(args, speaker)
	local x = args[1]
	if not tonumber(x) then
		local x = Enum.HumanoidStateType[args[1]]
	end
...
czReg('drophats',{'drophat'},function(args, speaker)
	if speaker.Character then
		for _,v in pairs(speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()) do
			v.Parent = workspace
		end
...
czReg('deletehats',{'nohats','rhats'},function(args, speaker)
...
czReg('droptools',{'droptool'},function(args, speaker)
...
czReg('droppabletools',{},function(args, speaker)
	if speaker.Character then
...
czReg('reach',{},function(args, speaker)
	czRun('unreach')
	wait()
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
...
czReg("boxreach", {}, function(args, speaker)
	czRun("unreach")
	wait()
	for i, v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
...
czReg('unreach',{'noreach','unboxreach'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
...
czReg('grippos',{},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
...
czReg('usetools', {}, function(args, speaker)
...
czReg("logs", {}, function(args, speaker)
	logsEnabled = true
	jLogsEnabled = true
	Toggle.Text = "Enabled"
	Toggle_2.Text = "Enabled"
...
czReg("chatlogs", {"clogs"}, function(args, speaker)
	logsEnabled = true
	join.Visible = false
	chat.Visible = true
	table.remove(czShadeC, table.find(czShadeC, selectChat))
...
czReg("joinlogs", {"jlogs"}, function(args, speaker)
	jLogsEnabled = true
	chat.Visible = false
	join.Visible = true	
	table.remove(czShadeC, table.find(czShadeC, selectJoin))
...
czReg("chatlogswebhook", {"logswebhook"}, function(args, speaker)
	if not httprequest then
		return czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn request)")
	end
...
flinging = false
czReg('fling',{},function(args, speaker)
	flinging = false
	...
	czRun('noclip nonotify')
	task.wait(.1)
	local bambam = Instance.new("BodyAngularVelocity")
	bambam.Name = czRandName()
...
czReg('unfling',{'nofling'},function(args, speaker)
	czRun('clip nonotify')
...
czReg('togglefling',{},function(args, speaker)
	if flinging then
		czRun('unfling')
	else
		czRun('fling')
...
czReg("flyfling", {}, function(args, speaker)
	czRun("unvehiclefly\\unwalkfling")
	task.wait()
	vehicleflyspeed = tonumber(args[1]) or vehicleflyspeed
	czRun("vehiclefly\\walkfling")
end)

czReg("unflyfling", {}, function(args, speaker)
	czRun("unvehiclefly\\unwalkfling\\breakvelocity")
end)

czReg("toggleflyfling", {}, function(args, speaker)
	czRun(flinging and "unflyfling" or "flyfling")
end)

walkflinging = false
czReg("walkfling", {}, function(args, speaker)
	czRun("unwalkfling")
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
	if humanoid then
...
czReg("unwalkfling", {"nowalkfling"}, function(args, speaker)
	walkflinging = false
	czRun("unnoclip nonotify")
end)

czReg("togglewalkfling", {}, function(args, speaker)
	czRun(walkflinging and "unwalkfling" or "walkfling")
end)

czReg('invisfling',{},function(args, speaker)
...
czReg("antifling", {}, function(args, speaker)
	if antifling then
...
czReg("unantifling", {}, function(args, speaker)
	if antifling then
		antifling:Disconnect()
		antifling = nil
	end
...
czReg("toggleantifling", {}, function(args, speaker)
	czRun(antifling and "unantifling" or "antifling")
end)
...
function attach(speaker,target)
	if tools(speaker) then
		...
		czToast('Tool Required','You need to have an item in your inventory to use this command')
	end
end
...
czReg("handlekill", {"hkill"}, function(args, speaker)
	if not firetouchinterest then
		return czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn firetouchinterest)")
	end
	if not speaker.Character then return end
	local tool = speaker.Character:FindFirstChildWhichIsA("Tool")
	local handle = tool and tool:FindFirstChild("Handle")
	if not handle then
		return czToast("Handle Kill", "You need to hold a \"Tool\" that does damage on touch. For example a common Sword tool.")
	end
	local range = tonumber(args[2]) or math.huge
	if range ~= math.huge then czToast("Handle Kill", ("Started!\nRadius: %s"):format(tostring(range):upper())) end
	...
	czToast("Handle Kill", "Stopped!")
end)

tpwalkStack = 0
czReg("teleportwalk", {"tpwalk"}, function(args, speaker)
...
czReg("unteleportwalk", {"untpwalk"}, function(args, speaker)
    tpwalkStack = 0
    tpwalking:Disconnect()
end)
...
czReg('spin',{},function(args, speaker)
	local spinSpeed = 20
	if args[1] and isNumber(args[1]) then
		spinSpeed = args[1]
	end
...
czReg('unspin',{},function(args, speaker)
	for i,v in pairs(getRoot(speaker.Character):GetChildren()) do
		if v.Name == "Spinning" then
			v:Destroy()
		end
...
czReg("xray", {}, function(args, speaker)
	xrayEnabled = true
	xray()
end)

czReg("unxray", {"noxray"}, function(args, speaker)
	xrayEnabled = false
	xray()
end)

czReg("togglexray", {}, function(args, speaker)
	xrayEnabled = not xrayEnabled
	xray()
end)

czReg("loopxray", {}, function(args, speaker)
...
czReg("unloopxray", {}, function(args, speaker)
	pcall(function() xrayLoop:Disconnect() end)
	xrayEnabled = false
	xray()
end)
...
czReg('walltp',{},function(args, speaker)
	local torso
	if r15(speaker) then
...
czReg('unwalltp',{'nowalltp'},function(args, speaker)
	if walltpTouch then
...
autoclicking = false
czReg('autoclick',{},function(args, speaker)
	if mouse1press and mouse1release then
		...
		czToast('Auto Clicker',"Press [backspace] and [=] at the same time to stop")
		...
		czToast('Auto Clicker',"Your exploit doesn't have the ability to use the autoclick")
	end
end)

czReg('unautoclick',{'noautoclick'},function(args, speaker)
	autoclicking = false
	if cancelAutoClick then cancelAutoClick:Disconnect() end
end)

czReg('mousesensitivity',{'ms'},function(args, speaker)
	UserInputService.MouseDeltaSensitivity = args[1]
end)

local nameBox = nil
local nbSelection = nil
czReg('hovername',{},function(args, speaker)
	czRun('unhovername')
	wait()
	nameBox = Instance.new("TextLabel")
	nameBox.Name = czRandName()
	nameBox.Parent = CzRootScale
	...
	nbUpdateFunc = CzMouse.Move:Connect(updateNameBox)
end)

czReg('unhovername',{'nohovername'},function(args, speaker)
...
czReg('headsize',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
...
czReg('hitbox',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
...
czReg("hitboxes", {}, function(args, speaker)
    settings():GetService("RenderSettings").ShowBoundingBoxes = true
end)

czReg("unhitboxes", {}, function(args, speaker)
    settings():GetService("RenderSettings").ShowBoundingBoxes = false
end)

czReg('stareat',{'stare'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		if stareLoop then
...
czReg('unstareat',{'unstare','nostare','nostareat'},function(args, speaker)
	if stareLoop then
		stareLoop:Disconnect()
	end
end)
	...
	if player:IsInGroup(RolewatchData.Group) then
		if tostring(player:GetRoleInGroup(RolewatchData.Group)):lower() == RolewatchData.Role:lower() then
			if RolewatchData.Leave == true then
				...
				czToast("Rolewatch", "Player \"" .. tostring(player.Name) .. "\" has joined with the Role \"" .. RolewatchData.Role .. "\"")
			end
		end
	end
end)

czReg("rolewatch", {}, function(args, speaker)
	...
	if groupId and roleName then
		...
		czToast("Rolewatch", "Watching Group ID \"" .. tostring(groupId) .. "\" for Role \"" .. roleName .. "\"")
	end
end)

czReg("rolewatchstop", {}, function(args, speaker)
	RolewatchData.Group = 0
	RolewatchData.Role = ""
	RolewatchData.Leave = false
	czToast("Rolewatch", "Disabled")
end)

czReg("rolewatchleave", {"unrolewatch"}, function(args, speaker)
	RolewatchData.Leave = not RolewatchData.Leave
	czToast("Rolewatch", RolewatchData.Leave and "Leave has been Enabled" or "Leave has been Disabled")
end)
...
czReg("staffwatch", {}, function(args, speaker)
	...
	if game.CreatorType == Enum.CreatorType.Group then
		...
		staffwatchjoin = Players.PlayerAdded:Connect(function(player)
			...
			if result.Staff then
				czToast("Staffwatch", formatUsername(player) .. " is a " .. result.Role)
			end
		end)
		for _, player in pairs(Players:GetPlayers()) do
			local result = getStaffRole(player)
		...
		if #found > 0 then
			czToast("Staffwatch", table.concat(found, ",\n"))
		else
			czToast("Staffwatch", "Enabled")
		end
	else
		czToast("Staffwatch", "Game is not owned by a Group")
	end
end)

czReg("unstaffwatch", {}, function(args, speaker)
	if staffwatchjoin then
		staffwatchjoin:Disconnect()
	end
	czToast("Staffwatch", "Disabled")
end)
...
czReg("findfriendgroups", {}, function(args, speaker)
    czToast("Checking Players", "This might take a while (slow function)")
...
czReg('removeterrain',{'rterrain','noterrain'},function(args, speaker)
	workspace:FindFirstChildOfClass('Terrain'):Clear()
end)

czReg('clearnilinstances',{'nonilinstances','cni'},function(args, speaker)
	if getnilinstances then
		for i,v in pairs(getnilinstances()) do
			v:Destroy()
		end
	else
		czToast('Incompatible Exploit','Your exploit does not support this command (czSafeFn getnilinstances)')
	end
end)

czReg('destroyheight',{'dh'},function(args, speaker)
...
czReg("antivoid", {}, function(args, speaker)
	czRun("unantivoid nonotify")
	task.wait()
	...
	if args[1] ~= "nonotify" then czToast("antivoid", "Enabled") end
end)

czReg("unantivoid", {"noantivoid"}, function(args, speaker)
	pcall(function() antivoidloop:Disconnect() end)
	antivoidloop = nil
	if args[1] ~= "nonotify" then czToast("antivoid", "Disabled") end
end)

antivoidWasEnabled = false
czReg("fakeout", {}, function(args, speaker)
	local root = getRoot(speaker.Character)
	...
	if antivoidWasEnabled then
		czRun("antivoid nonotify")
		antivoidWasEnabled = false
	end
end)

czReg("trip", {}, function(args, speaker)
	local humanoid = speaker.Character and speaker.Character:FindFirstChildWhichIsA("Humanoid")
	local root = speaker.Character and getRoot(speaker.Character)
...
czReg("removeads", {"adblock"}, function(args, speaker)
	while wait() do
		pcall(function()
			for i, v in pairs(workspace:GetDescendants()) do
...
czReg("scare", {"spook"}, function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local oldpos = nil
...
czReg("alignmentkeys", {}, function(args, speaker)
...
czReg("unalignmentkeys", {"noalignmentkeys"}, function(args, speaker)
...
czReg("ctrllock", {}, function(args, speaker)
...
czReg("unctrllock", {}, function(args, speaker)
	local mouseLockController = speaker.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("CameraModule"):WaitForChild("MouseLockController")
	local boundKeys = mouseLockController:FindFirstChild("BoundKeys")
...
czReg("listento", {}, function(args, speaker)
	czRun("unlistento")
	if not args[1] then return end
...
czReg("unlistento", {}, function(args, speaker)
	SoundService:SetListener(Enum.ListenerType.Camera)
	listentoChar:Disconnect()
end)

czReg("jerk", {}, function(args, speaker)
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
...
czReg("guiscale", {}, function(args, speaker)
	if args[1] and isNumber(args[1]) then
	...
	Scale.Scale = math.max(CzPanel.AbsoluteSize.X / 1920, guiScale)
	czSaveCfg()
end)

czReg("muteallvoices", {"muteallvcs"}, function(args, speaker)
	CzServices.VoiceChatInternal:SubscribePauseAll(true)
end)

czReg("unmuteallvoices", {"unmuteallvcs"}, function(args, speaker)
	CzServices.VoiceChatInternal:SubscribePauseAll(false)
end)

czReg("mutevc", {}, function(args, speaker)
	for _, plr in getPlayer(args[1], speaker) do
		if Players[plr] == speaker then continue end
		CzServices.VoiceChatInternal:SubscribePause(Players[plr].UserId, true)
	end
end)

czReg("unmutevc", {}, function(args, speaker)
	for _, plr in getPlayer(args[1], speaker) do
		if Players[plr] == speaker then continue end
		CzServices.VoiceChatInternal:SubscribePause(Players[plr].UserId, false)
	end
end)

czReg("phonebook", {"call"}, function(args, speaker)
	...
	if success and canInvite then
		...
		czToast("Phonebook", "It seems you're not able to call anyone. Sorry!")
	end
end)

local freezingua = nil
frozenParts = {}
czReg('freezeunanchored',{'freezeua'},function(args, speaker)
	local badnames = {
		"Head",
		"UpperTorso",
		"LowerTorso",
...
czReg('thawunanchored',{'thawua','unfreezeunanchored','unfreezeua'},function(args, speaker)
	if freezingua then
...
czReg('tpunanchored',{'tpua'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local Forces = {}
...
czReg('autokeypress',{'keypress'},function(args, speaker)
	if keypress and keyrelease and args[1] then
		local code = keycodeMap[args[1]:lower()]
		if not code then czToast('Auto Key Press',"Invalid key") return end
		czRun('unautokeypress')
		wait()
		local clickDelay = 0.1
		local releaseDelay = 0.1
		...
		czToast('Auto Key Press',"Press [backspace] and [=] at the same time to stop")
		repeat wait(clickDelay)
			keypress(code)
			wait(releaseDelay)
			keyrelease(code)
		...
		czToast('Auto Key Press',"Your exploit doesn't have the ability to use auto key press")
	end
end)

czReg('unautokeypress',{'noautokeypress','unkeypress','nokeypress'},function(args, speaker)
	autoKeyPressing = false
	if cancelAutoKeyPress then cancelAutoKeyPress:Disconnect() end
end)

czReg('addplugin',{'plugin'},function(args, speaker)
	addPlugin(getstring(1, args))
end)

czReg('removeplugin',{'deleteplugin'},function(args, speaker)
	deletePlugin(getstring(1, args))
end)

czReg('reloadplugin',{},function(args, speaker)
...
czReg("addallplugins", {"loadallplugins"}, function(args, speaker)
	if not listfiles or not isfolder then
		czToast("Incompatible Exploit", "Your exploit does not support this command (czSafeFn listfiles/isfolder)")
		return
	end
...
czReg('czUnreg',{'deletecmd'},function(args, speaker)
	czUnreg(args[1])
end)

czReg("debug", {}, function(args, speaker)
    local opt = parseBoolean(args[1], true)
    _G.CZASX_VERBOSE = opt
    czToast("debug", tostring(opt), 1)
end)

if czMobile then
	local QuickCapture = Instance.new("TextButton")
	local UICorner = Instance.new("UICorner")
	QuickCapture.Name = czRandName()
	QuickCapture.Parent = CzGuiParent
	QuickCapture.BackgroundColor3 = Color3.fromRGB(14, 16, 22)
	...
	table.insert(czShadeA, QuickCapture)
	table.insert(czTextA, QuickCapture)
end

pcall(function() Scale.Scale = math.max(CzPanel.AbsoluteSize.X / 1920, guiScale) end)
Scale.Parent = CzRootScale
CzRootScale.Size = UDim2.fromScale(1 / Scale.Scale, 1 / Scale.Scale)
Scale:GetPropertyChangedSignal("Scale"):Connect(function()
	CzRootScale.Size = UDim2.fromScale(1 / Scale.Scale, 1 / Scale.Scale)
	for _, v in CzRootScale:GetDescendants() do
...
updateColors(currentShade1,czShadeA)
updateColors(currentShade2,czShadeB)
updateColors(currentShade3,czShadeC)
updateColors(currentText1,czTextA)
updateColors(currentText2,czTextB)
updateColors(currentScroll,czScrollBag)
...
Players.PlayerAdded:Connect(function(plr)
	...
	if czLegacyChat then plr.Chatted:Connect(function(msg) eventEditor.FireEvent("OnChatted",tostring(plr),msg) end) end
	plr.CharacterAdded:Connect(function() eventEditor.FireEvent("OnSpawn",tostring(plr)) hookCharEvents(plr) end)
	JoinLog(plr)
	if czLegacyChat then ChatLog(plr) end
	if ESPenabled then
	...
	TextChatService.MessageReceived:Connect(function(message)
		if message.TextSource and message.Status ~= Enum.TextChatMessageStatus.InvalidTextChannelPermissions then
			...
			if player.UserId == Players.LocalPlayer.UserId then
				czDoRun(message.Text, Players.LocalPlayer)
			end
...
CzMouse.Move:Connect(checkTT)

CaptureService.CaptureBegan:Connect(function()
	CzGuiParent.Enabled = false
end)

CaptureService.CaptureEnded:Connect(function()
	task.delay(0.1, function()
		CzGuiParent.Enabled = true
	end)
end)

task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = nil -- version pin disabled
		return HttpService:JSONDecode(versionJson)
	end)

	if success then
		if CZ_VER ~= latestVersionInfo.Version then
			czToast("Czasx", "Running build " .. CZ_VER)
		end
		...
		if latestVersionInfo.Announcement and latestVersionInfo.Announcement ~= "" then
			...
			AnnGUI.Name = czRandName()
			AnnGUI.Parent = CzRootScale
			AnnGUI.Active = true
...
================================================================
  END OF CZASX SOURCE
  Watermark : Czasx Engine | 2.4.1-cz
  Config    : czasx_cfg.dat
  Modes     : EXEC = run cmds | FIND = search only
================================================================
]]
