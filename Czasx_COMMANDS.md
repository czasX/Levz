...
| Command | Mô tả |
|---------|--------|
| `discord / support / help` | Invite to the Czasx discord server. |
| `guiscale [number]` | Changes the size of the gui. [number] accepts decimals and whole numbers. Min is 0.4 and Max is 2 |
| `console` | Opens the Roblox console |
| `oldconsole` | Loads an old-themed Roblox console |
| `explorer / dex` | Opens Dex++ by Chillz |
| `moondex / mdex` | Opens DEX by Moon |
| `remotespy / rspy` | Opens Cobalt to intercept incoming and outgoing network traffic |
| `simplespy / sspy` | Opens Simple Spy V3 to intercept remote calls from the client to the server |
...
| `rejoin / rj [reposition]` | Makes you rejoin the game |
| `inviteprompt` | Prompts the user of your choice that they had invited you. Requires a rejoin to function. |
...
| `hideguis` | Hides any GUIs in CzPlayerGui |
| `unhideguis` | Undoes hideguis |
| `guidelete` | Enables backspace to delete GUI |
| `unguidelete / noguidelete` | Disables guidelete |
| `hidecz` | Hides the main IY GUI |
| `showcz / unhidecz` | Shows IY again |
| `keepcz` | Auto execute IY when you teleport through servers |
| `unkeepcz` | Disable keepiy |
| `togglekeepcz` | Toggles keepiy |
...
| `antigameplaypaused` | Clears the annoying box shown when a game is loading assets due to network lag |
| `unantigameplaypaused` | Disables antigameplaypaused |
...
| `allowrejoin / allowrj (CLIENT)` | Toggles if antiteleport allows you to rejoin or not |
...
| `czToast [text]` | Sends you a notification with the provided text |
...
| `alignmentkeys` | Enables the left and right alignment keys (comma and period) |
| `unalignmentkeys / noalignmentkeys` | Disables the alignment keys |
| `ctrllock` | Binds Shiftlock to LeftControl |
| `unctrllock` | Re-binds Shiftlock to LeftShift |
| `exit / shutdown / leave` | Exits the game |
| `czUnreg / deletecmd` | Removes a command until the script is reloaded |
...
| `goto [player] [request stream]` | Go to a player |
...
| `pulsetp / ptp [player] [seconds]` | Teleports you to a player for a specified amount of time |
...
| `anchor` | Anchors your characters RootPart |
| `unanchor` | Unanchors your characters RootPart |
...
| `thru [num]` | Teleports you [num] studs ahead of where your character is facing |
...
| `chatwindow` | Enables the chat window for your client |
| `unchatwindow / nochatwindow` | Disables the chat window for your client |
| `darkchat` | Makes the chat window dark for your client |
| `listento [player]` | Listens to the area around a player. Can also eavesdrop with vc |
| `unlistento` | Disables listento |
| `muteallvoices / muteallvcs` | Mutes voice chat for all players |
| `unmuteallvoices / unmuteallvcs` | Unmutes voice chat for all players |
| `mutevc [player]` | Mutes the voice chat of a player |
| `unmutevc [player]` | Unmutes the voice chat of a player |
| `phonebook / call` | Prompts the Roblox phonebook UI to let you call your friends. Needs voice chat enabled |
| `esp` | View all players and their status |
| `espteam` | ESP but teammates are green and bad guys are red |
| `noesp / unesp / unespteam` | Removes ESP |
...
| `unxray / noxray` | Restores transparency to all parts in workspace |
| `loopxray` | Makes all parts in workspace transparent but looped |
| `unloopxray` | Unloops xray |
| `togglexray` | Toggles xray |
...
| `copyplaceid / placeid` | Copies the current place id to your clipboard |
| `copygameid / gameid` | Copies the current game id to your clipboard |
...
| `unbang` | uwu |
| `jerk` | Makes you jork it |
| `scare / spook [player]` | Teleports in front of a player for half a second |
...
| `findfriendgroups` | Notifies you if any players are friends with each other |
| `handlekill / hkill [player] [radius] (TOOL)` | Kills a player using tool damage (YOU NEED A TOOL) |
...
| `hitboxes` | Shows all rendered bounding boxes |
| `unhitboxes` | Stops showing all rendered bounding boxes |
...
| `speed / ws / walkspeed [num]` | Change your walkspeed (default is 16) |
...
| `jumppower / jpower / jp [num]` | Change a players jump height (default is 50) |
...
| `animation / anim [ID] [speed]` | Makes your character perform an animation (must be an animation on the marketplace or by roblox/stickmasterluke to replicate) |
| `emote / em [ID] [speed]` | Makes your character perform an emote (must be on the marketplace or by roblox/stickmasterluke to replicate) |
...
| `clickdelete` | Go to CzSettings > Keybinds > Add for click delete |
| `clickteleport` | Go to CzSettings > Keybinds > Add for click teleport |
...
| `dupetools / clonetools [num]` | Duplicates your inventory tools a set amount of times |
...
| `usetools [amount] [delay]` | Activates all tools in your backpack at the same time |
